#' @title Species2Char
#' @description Summary the expression in \bold{exp_data} from lipid species to user-selected characteristics.
...
#' @return Return \bold{transform_table}. An data frame of aggregated(sum) expression data by user-selected characteristics (\bold{char_var}).
#' @export
#' @examples
#' \dontrun{
#' data("DE_exp_data")
#' exp_data <- DE_exp_data
#' lipid_char_table <- data("DE_lipid_char_table")
...
Species2Char <- function(exp_data, lipid_char_table, char_var){
  ...
  if(stringr::str_detect(char_var, 'FA_')){
    max_chain_num <- stringr::str_split(lipid_char_table[[char_var]],  pattern = ',') %>%
      purrr::map_dbl(length) %>% max()

    suppressWarnings(
      transform_table <- exp_data %>% dplyr::left_join(lipid_char_table[c('feature', char_var)], by='feature') %>%
        tidyr::separate(eval(parse(text = char_var)), letters[1:max_chain_num]) %>%
        tidyr::gather(letters[1:max_chain_num], key='key', value = 'value') %>%
    ...
    if(nrow(transform_table)==0){
      ...
      transform_table <- transform_table %>%
        stats::aggregate(stats::as.formula(stringr::str_c('. ~ ',char_var)), ., sum)
    }
  }
