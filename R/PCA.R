...
PCA <- function(exp_transform_table, group_info = NULL, sig_feature = NULL,
  ...
  if(TRUE){
    ...
    colnames(pca_contrib_table)[seq(2,ncol(pca_contrib_table))] <- stringr::str_c('PC',as.character(seq_len(PC)))
  ...
  feature_contrib <- factoextra::fviz_contrib(pca, choice = "var", addlabels=TRUE,axes = n_PC, top = top_n_feature,
  ...
  if(top_n_feature>ncol(pca$rotation)){
    ...
    warning_message <- paste0("top_n_feature will only show ",ncol(pca$rotation))
