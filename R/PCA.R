#' @title PCA
#' @description PCA (Principal Component Analysis):  A method of dimensionality reduction to transform data from a high-dimensional space into a low-dimensional space so that the vital properties of the original data can be retained.
#' @description This function calculates Principal component analysis with the classical prcomp function and visualizes results by PCA plot and scree plot. Furthermore, this function also visualizes the features' contribution of the user-defined principal components and the correlation between a feature (lipid species) and a principal component (PC).
...
#' @param scaling A logical value. If scaling = TRUE, each block is standardized to zero means and unit variances. (default: TRUE).
#' @param centering A logical value indicating whether the variables should be shifted to be zero centered. Alternately, a vector of length equal the number of columns of x can be supplied. The value is passed to scale.
#' @param cluster_method A character string indicating which method to be used for clustering. Allowed method include \bold{'kmeans'}, \bold{'kmedoids'}, \bold{'hclustering'}, \bold{'dbscan'}, \bold{'group_info'}. (default: 'kmeans')
#' @param group_num A positive integer specifying the number of clusters. The number must be between 1 and 10.
...
#' @param top_n_feature A numeric value specifying the number of top elements to be shown.
#' @return Return a list with 1 model, 2 data frame, and 4 plots.
#' \enumerate{
#' \item pca: prcomp (lists of the standard deviations, the matrix of variable loadings, the value of the rotated data, the centering and scaling )
...
#' \item feature_contrib: plot, the contribution of top N features of the user-defined principal components.
...
#' exp_transform_table <- data_process(exp_data, exclude_var_missing=T, missing_pct_limit=50,
...
#' PCA(exp_transform_table, group_info = NULL, sig_feature = NULL, scaling=T, centering=T,
#'     cluster_method='kmeans', group_num = 2, var1 = NULL, var2 = NULL, insert_ref_group=NULL,
#'     ref_group=NULL, n_PC=c(1,2), top_n_feature=10)
#' }
PCA <- function(exp_transform_table, group_info = NULL, sig_feature = NULL,
  ...
  #output pca data
  pca_rotated_data <- as.data.frame(pca$x) %>% dplyr::mutate(sample_name=colnames(exp_transform_table)[-1])
  ...
  if(cluster_method=='kmeans'){
    ...
    group_order <- purrr::map_dbl(rownames(pca_table), function(x){which(x==group_info$sample_name)})
  ...
  pca_rotated_data <- pca_rotated_data %>% dplyr::mutate(group=cluster_group) %>%
  ...
  if(T){
    ...
    pca_contrib_table <- factoextra::get_pca_var(pca)$contrib %>% as.data.frame() %>%
      dplyr::mutate(feature=rownames(.)) %>% dplyr::select(feature, dplyr::everything())

    PC <- ncol(pca_contrib_table)-1
    colnames(pca_contrib_table)[2:ncol(pca_contrib_table)] <- stringr::str_c('PC',as.character(1:PC))

    #pca plot*5
    color <- c("#00AFBB", "#E7B800", "#FC4E07","#42B540FF","#BB3099","#EE0099","#0000AC","#868686FF",'#00468BFF','black')

    if(cluster_method %in% c('dbscan')){
      pca_biplot <- factoextra::fviz_pca_ind(pca,label = "none", habillage = cluster_group,
    ...
    pca_screeplot <- factoextra::fviz_screeplot(pca, ncp=10, main="Scree plot for Top10 PCs",
                                    xlab="Principle components (PCs)",
                                    ylab='Explained variance (%)')
    pca_screeplot <- plotly::ggplotly(pca_screeplot) %>%
      plotly::style(traces = 1:2,hoverinfo="text",
            text=paste("Principle components :",pca_screeplot$data$dim,
                       "\nExplained variance :",round(pca_screeplot$data$eig,3),"%"))

  }
  feature_contrib <- factoextra::fviz_contrib(pca, choice = "var", addlabels=T,axes = n_PC, top = top_n_feature,
                                  title=stringr::str_c('Contribution of Top',as.character(top_n_feature),
                                              ' features to PC-', stringr::str_c(as.character(n_PC), collapse = ',')))
  ...
  pca_variable <- factoextra::fviz_pca_var(pca, col.var="contrib",select.var= list(contrib = top_n_feature),
  ...
  if(top_n_feature>ncol(pca$rotation)){
    ...
    warning(paste0("top_n_feature will only show ",ncol(pca$rotation)))
  }
  for (i in 1:top_n_feature) {
    pca_variable_ggplotly <- pca_variable_ggplotly %>%
      plotly::add_annotations(data = pca_variable$data[i,],x = ~x, y = ~y,text="",
