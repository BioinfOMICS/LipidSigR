...
.check_abundance <- function(abundance, se_type){
    ...
    if (nrow(abundance_rmR) < nrow(abundance_mat)) {
        ...
        if (isTRUE(.check_rowNum(abundance_mat, 2))) {
            warning("Remove ", rm_rowNum, "/", nrow(abundance), " features with constant values (including all NAs and all zeros).")
        } else {
            ## after remove the features is not enough for analysis
            stop("Remove ", rm_rowNum, "/", nrow(abundance), " features with constant values (including all NAs and all zeros). No features for further analysis.")
        }
    }
