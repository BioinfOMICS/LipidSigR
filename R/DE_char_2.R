#' @title DE_char_2
#' @description
#' The \code{\link{DE_sub_char_2}} function is for lipid characteristics 
#'     analysis, which is used to search for deferentially expressed lipid 
#'     characters based on the user's choice. In this function, two-way ANOVA 
#'     will be used to evaluate the interaction between lipid characteristics 
#'     (class, total length, etc.) and group information 
#'     (control v.s experimental group) on the lipid expression values or 
#'     percentages (according to selected characteristics). 
#'     To obtain the overview of differentially expressed lipids by 
#'     user-selected characteristics, an expression table from 
#'     \code{\link{Species2Char}} and a lipid characteristic table are needed. 
#'     Note that the sample can be paired or not, depending on 
#'     \bold{paired=TRUE/FALSE}, and this function can further be used for 
#'     subgroup analysis, \code{\link{DE_sub_char_2}}.
#' \enumerate{
...
#' }
#' @param exp_data A data frame with a list of lipid caracteristics in the first
#'     row and sample names as column name with expression as value. 
...
#'     users from the column name of \bold{lipid_char_table}.
#' @param group_info A data frame according to Group information table 
#'     (after convert to ctrl and exp). NAs are allowed.
...
#' \enumerate{
#' \item DE_char_exp_data: data frame, the expression of the user-selected 
#'     lipid characteristics by samples.
#' \item DE_char_table_all: data frame with statistical analysis for 
#'     differentially expressed characters. For example, the result of 
#'     two-way ANOVA will tell you whether the interaction effect between 
#'     totallength and different groups is present. The post hoc test (t-test) 
#'.    will then calculate significance for each length of FA and 
#'     produce a p-value.
#' \item DE_char_combined_table: data frame with the value calculated by the 
#'     weighted average of the expression of the continuous 
#'     lipid characteristics. 
#'     \emph{NOTE: this output is used for DE_char_trendplot}.
#' \item DE_char_combine_result_table: data frame with statistics of t.test 
#'     for control and experiment groups.
#' \item DE_char_barplot: a barplot shows the average of each sample group 
#'     and whether they are significantly differentially expressed between 
#'     two groups based on user-selected character.
#' \item DE_char_barplot_sqrt: a barplot shows the average of each sample group 
#'     and whether they are significantly differentially expressed between 
#'     two groups based on user-selected character. 
#'     \emph{NOTE: the y axis is sqrt-scaled}.
#' \item DE_char_boxplot: plot with the index value from DE_char_combined_table 
#'     and DE_char_combine_result_table.
#' \item DE_char_trendplot: a line chart shows the average of each sample group 
#'     and whether they are significantly differentially expressed between 
#'     two groups based on user-selected character.
#' \item DE_char_trendplot_sqrt: a line shows the average of each sample group 
#'     and whether they are significantly differentially expressed between 
#'     two groups based on user-selected character. 
#'     \emph{NOTE: the y axis is sqrt-scaled}.
#' }
...
#' exp_data_Spe2Char <- Species2Char(exp_data,
...
#' DE_char_2(exp_transform_non_log, data_transform = TRUE,
#'           group_info = group_info, paired = FALSE,
#'           sig_pvalue = 0.05, sig_FC = 2, insert_ref_group=NULL,
#'           ref_group=NULL)
DE_char_2 <- function(exp_data,
                      data_transform=TRUE,
                      group_info,
                      paired=FALSE,
                      sig_pvalue=0.05,
  ...
  if(ncol(group_info) == 4){
    if(sum(vapply(group_info[, seq_len(3)], class, character(1))
           != "character") == 0){
    ...
    if(sum(!is.na(group_info[, 4])) != 0 | 
      ...
      stop("group_info each pair must have a specific number,
           staring from 1 to N. Cannot have NA, blank, or skip numbers.")
    }
    if(sum(group_info[, 1] %in% colnames(exp_data)) != nrow(group_info) | 
       sum(group_info[, 1] %in% colnames(exp_data)) != ncol(exp_data[, -1])){
      stop("group_info 'sample_name' must same as 
           the name of samples of exp_data")
    }
    ...
    if(!is.null(insert_ref_group)){
      if(!insert_ref_group %in% group_info[, 3]){
        stop("The insert_ref_group entered by users must be 
             included in the group_info.")
      }
    }
  }
  ...
  barTab_sig <- barTab %>% 
    ...
    dplyr::mutate(pvalue_text=ifelse(post_hoc_pvalue <= 0.001, "***",
  ...
  if(plotly == TRUE){
    ...
    for (i in seq_len(length(unique(barPlot$data$Group)))) {
      ...
      data <- barPlot$data[
        which(barPlot$data$Group == unique(barPlot$data$Group)[i]),]
  ...
  if(plotly == TRUE){
    ...
    for (i in seq_len(length(unique(barPlot_sqrt$data$Group)))) {
      ...
      data <- barPlot_sqrt$data[
        which(barPlot_sqrt$data$Group == unique(barPlot_sqrt$data$Group)[i]),]
  ...
  if(nrow(Combined_char_data) > 0 & nrow(Combine_char_result_table) > 0){
    ...
    if(plotly == TRUE){
      ...
      for (i in seq_len(length(unique(linePlot$data$Group)))) {
        ...
        data <- linePlot$data[
          which(linePlot$data$Group == unique(linePlot$data$Group)[i]),]
    ...
    if(plotly == TRUE){
      ...
      for (i in seq_len(length(unique(linePlot_sqrt$data$Group)))) {
        ...
        data <- linePlot_sqrt$data[
          which(linePlot_sqrt$data$Group == unique(linePlot_sqrt$data$Group)[i]),]
        ...
        if(sum(grepl("\\*",line_sqrt_ggplot$x$data[[i+n]]$text)) == 0){
          line_sqrt_ggplot$x$data[[i+n]]$text <- paste0("Category :", 
                                                        data$Category,
                                                        "\nMean :", 
                                                        round(data$Mean, 3),
                                                        "\nSD :", 
      ...
      for (i in seq_len(length(line_sqrt_ggplot$x$data))) {
        text <- stringr::str_split(line_sqrt_ggplot$x$data[[i]]$hovertext, 
                                   "<br />max_error_bar")
        hovertext <- list()
        if(length(text) > 0){
          for (j in seq_len(length(text))) {
    ...
    if(Combine_char_result_table$sig == 'yes'){
      if(plotly == TRUE){
        ...
        if(Combine_char_result_table$p_value <= 0.05 & 
           Combine_char_result_table$p_value > 0.01){
          t.text <- c('', '*', '')
        }else if (Combine_char_result_table$p_value <= 0.01 & 
                  Combine_char_result_table$p_value > 0.001){
          t.text <- c('', '**', '')
        }else{
          t.text <- c('', '***', '')
        }
        ...
        boxPlot <- ggboxplot(boxTab, x = "group", y = "Category", 
                             color = "group", 
                             add = "jitter") +
          stat_compare_means(method = "t.test") + 
          scale_color_manual(values=c("lightslateblue","sienna2")) +
          ggplot2::labs(y=paste0(CHAR, ' index'),
                        x='Group') + 
