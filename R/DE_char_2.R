...
#' @param paired Logical. If \bold{paired = TRUE}, data are paired samples. (default: FALSE)
...
DE_char_2 <- function(exp_data,  data_transform=TRUE,
  ...
  for(a in seq_len(length(var))){
    ...
    if(paired==TRUE){
      pvalue[a] <- tryCatch(
        stats::t.test(unlist(Char_Transform_table[a,group1]), unlist(Char_Transform_table[a,group2]), paired = TRUE, var.equal = TRUE)$p.value,
        error=function(e){NA}
      )

    }else{
      pvalue[a] <- tryCatch(
        stats::t.test(unlist(Char_Transform_table[a,group1]), unlist(Char_Transform_table[a,group2]), paired = FALSE, var.equal = TRUE)$p.value,
        error=function(e){NA}
      )
    }
  ...
  if(sum(is.na(as.numeric(var)))==0){
    ...
    Combined_char_data <- purrr::map2(char_exp_data[1], char_exp_data[-1], ~sum(.x*.y, na.rm = TRUE)/sum(.y, na.rm = TRUE)) %>%
    ...
    if(paired==TRUE){
      pvalue <- tryCatch(
        stats::t.test(unlist(Combined_char_transform_table[,group1]), unlist(Combined_char_transform_table[,group2]), paired = TRUE, var.equal = TRUE)$p.value,
        error=function(e){NA}
      )

    }else{
      pvalue <- tryCatch(
        stats::t.test(unlist(Combined_char_transform_table[,group1]), unlist(Combined_char_transform_table[,group2]), paired = FALSE, var.equal = TRUE)$p.value,
        error=function(e){NA}
      )
    }
