...
.dataSpecificNodes <- function(
    ...
    lipid.class <- edges %>% dplyr::filter(arrows != '') %>%
        tidyr::gather(from_to, class, seq_len(2)) %>% dplyr::distinct(class) %>% .$class
...
.speciesNodeColor <- function(species.node){
    ...
    if(nrow(s.nodes.color.neg) > 0){
        ...
        for (i in seq_len(nrow(s.nodes.color.neg))){
            for (a in seq_len(s.n_color.neg)) {
    ...
    if(nrow(s.nodes.color.pos) > 0){
        ...
        for (i in seq_len(nrow(s.nodes.color.pos))) {
            for (a in seq_len(s.n_color.pos)) {
...
.classNodeColor <- function(class.node, sig_class){
    ...
    if(nrow(c.nodes.color.neg) > 0){
        ...
        for (i in seq_len(nrow(c.nodes.color.neg))){
            for (a in seq_len(c.n_color.neg)){
    ...
    if(nrow(c.nodes.color.pos) > 0){
        ...
        for (i in seq_len(nrow(c.nodes.color.pos))){
            for (a in seq_len(c.n_color.pos)){
