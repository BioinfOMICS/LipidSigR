...
.corr_heatmap <- function(
    ...
    if(length(unique(table_sig[["feature"]])) > 1){
        ...
        if(sum(is.na(Cor.mat))==0 & nrow(Cor.mat) >= 2 & ncol(Cor.mat) >= 2){
            ...
            static_hm <- ComplexHeatmap::Heatmap(
                matrix=Cor.mat, row_names_side="left", row_dend_side='right',
                column_names_side="bottom", col=.colorScale(Cor.mat), name='Signal',
...
.colorScale <- function(data){
    Min <- min(data)
    Max <- max(data)
    absMin <- abs(min(data))
    absMax <- abs(max(data))
    if(Max <= 0 ){
        colorScale <- circlize::colorRamp2(c(Min, 0), c("#0571b0", "white"))
    }else if(Min >= 0 ){
        colorScale <- circlize::colorRamp2(c(0, Max), c("white", "#ca0020"))
    }else{
        if(absMax >= absMin){
            if(absMax > 2 * absMin){
                colorScale <- circlize::colorRamp2(c(Min, 0, absMin ,Max), c("#92c5de", "white", "#f4a582", "#ca0020"))
            }else{
                colorScale <- circlize::colorRamp2(c(Min, 0 ,Max), c("#0571b0", "white", "#ca0020"))
            }
        }else if(absMax < absMin){
            if(absMin > 2 * absMax){
                colorScale <- circlize::colorRamp2(c(Min, -Max, 0, Max), c("#0571b0", "#92c5de", "white", "#f4a582"))
            }else{
                colorScale <- circlize::colorRamp2(c(Min, 0, Max), c("#0571b0", "white", "#ca0020"))
            }
        }
