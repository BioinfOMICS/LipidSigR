...
#' @description
#' \enumerate{
#' \item Determine whether significant lipid species are enriched in user-selected characteristics.
...
#' @param lipid_char_table A data frame with lipid features, such as class, total length. NAs are allowed. The name of first column must be "feature".
...
#' \enumerate{
#' \item enrich_char_table: a data frame encompasses condition, characteristic, p-value, and significance.
#' \item enrich_char_barplot: the enrichment plot for up/down/non-significant groups
#' }
#' @export
#' @examples
#' \dontrun{
...
#'                                       pct_transform=T, data_transform=F,
#'                                       trans_type='log', centering=F, scaling=F)
...
Enrichment <- function(DE_species_table_sig, lipid_char_table, char_var, sig_pvalue = 0.05){
  ...
  sig_class.all <- lipo.fisher(sig_lipid.all, lipo.category) %>% dplyr::mutate(condition='UP & DOWN')
  if(nrow(sig_lipid.up) > 0){
    sig_class.up <- lipo.fisher(sig_lipid.up, lipo.category) %>% dplyr::mutate(condition='UP')
  }else{
    sig_class.up <- NULL
  }
  if(nrow(sig_lipid.down) > 0){
    sig_class.down <- lipo.fisher(sig_lipid.down, lipo.category) %>% dplyr::mutate(condition='DOWN')
  }else{
    sig_class.down <- NULL
  }
  ...
  if(max(plot.tab$m.log.p) == 0){
    ...
    if(length(unique(plot.tab$condition)) == 2){
      ...
      plot.tab <- plot.tab %>% dplyr::group_by(characteristic) %>% dplyr::mutate(rank=mlogP[which(abs(mlogP)==max(abs(mlogP)))])
      p.sig.class <- ggplot2::ggplot(plot.tab,
                                     ggplot2::aes(x=mlogP, y=stats::reorder(characteristic, rank,max), fill=significance)) +
        ggplot2::geom_col() +
        ggplot2::geom_vline(xintercept = 0, color = '#444444') +
        ggthemes::theme_hc() +
        ggplot2::scale_fill_manual(breaks=c('DOWN', 'UP', 'non-significant'), values =c('#4169E1', '#FF6347', '#666666'))+
      ...
      p.sig.class <- ggplot2::ggplot(plot.tab,
                                     ggplot2::aes(x=mlogP, y=stats::reorder(characteristic, mlogP), fill=significance)) +
        ggplot2::geom_col() +
        ggthemes::theme_hc() +
        ggplot2::scale_fill_manual(breaks=c('DOWN', 'UP', 'non-significant'), values =c('#4169E1', '#FF6347', '#666666'))+
        ggplot2::theme(axis.title = ggplot2::element_text(size=14),
