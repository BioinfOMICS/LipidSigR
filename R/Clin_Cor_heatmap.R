...
#' @description The correlation coefficient provides a summary view of the
#' existing relationship between clinical features and lipid species,
...
#' @description This function returns the heatmap after users input the
#' cut-offs and choose a value for clustering/methods for clustering.
#' Users can use either correlation coefficient between clinical features
#' (e.g. genes) and lipid species or choose their statistic instead.
  ...
  if(sum(condition_table[, 1] %in% colnames(exp_data)) != nrow(condition_table)|
    ...
    if(length(unique(Clin_Cor_table_sig[[2]])) > 1){
      ...
      if(plotly == TRUE){
        ...
        stats::heatmap(Cor.mat, Rowv=TRUE, Colv=TRUE,
