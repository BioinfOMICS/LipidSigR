...
#'                    group_info = group_info, char_var = char_var[6],
#'                    split_var = char_var[2],
...
DE_sub_char_plot_2 <- function(DE_split_char_table_all, 
  ...
  if(plotly == TRUE){
    ...
    for (i in seq_len(length(unique(barPlot$data$Group)))) {
      ...
      data <- barPlot$data[which(
        barPlot$data$Group == unique(barPlot$data$Group)[i]),]
  ...
  if(plotly == TRUE){
    ...
    for (i in seq_len(length(unique(linePlot$data$Group)))) {
      ...
      data <- linePlot$data[which(
        linePlot$data$Group == unique(linePlot$data$Group)[i]),]
