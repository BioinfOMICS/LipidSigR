...
#' DE.sub.char.2 <- DE_sub_char_2(exp_data, data_transform=T, lipid_char_table = lipid_char_table,
#'                                split_var = char_var[1], char_var = char_var[4],
#'                                group_info = group_info,paired = F, sig_pvalue=0.05, sig_FC=2,
#'                                exclude_var_missing=T,missing_pct_limit=50, replace_zero=T,
#'                                zero2what='min', xmin=0.5,replace_NA=T, NA2what='min', ymin=0.5,
#'                                pct_transform=T, trans_type='log', centering=F, scaling=F)
#' char.class <- unique(DE.sub.char.2[[2]][1])
#' DE_sub_char_plot_2(DE.sub.char.2[[2]], DE_split_char_index = DE.sub.char.2[[3]],
#'                    group_info = group_info, char_var = char_var[4], split_var = char_var[1],
...
DE_sub_char_plot_2 <- function(DE_split_char_table_all, DE_split_char_index, group_info,
  ...
  splitTab <- splitTab %>% dplyr::group_by(Category) %>% dplyr::mutate(max_error_bar = max(Mean+SD)) %>% ungroup()
  ...
  t.test.pvalue <- tryCatch({stats::t.test(Category ~ group, data = boxTab, var.equal = T)["p.value"]},
