...
#' @description Select the user-selected characteristics, and make it become a long table.
...
#' @return Return 1 long matrix of lipid species with user-selected lipid characteristics.
#' @export
#' @examples
#' \dontrun{
...
lipid_char_table_gather <- function(lipid_char_table, char_var){
  ...
  if(!char_var %in% grep(pattern = 'FA_', x = colnames(lipid_char_table), value = T)){
