...
lipid_annotation <- function(goslin_annotation){
    ...
    if(all(goslin_annotation$Grammar == 'NOT_PARSEABLE')){
        stop("None of the lipid names are parseable by Goslin. Please check if",
    ...
    lipid.char <- lipid.char.all %>%
        dplyr::select(seq_len(13), seq(from=40, to=43),
                      seq(from=14, to=39), seq(from=47, to=62), 45, 63,
                      64, 46, seq(from=65, to=73)) %>%
        as.data.frame()
...
.modifyGoslinResults <- function(goslin_annotation){
    ...
    # Parseable Lipids
    goslin.res <- goslin_annotation %>% dplyr::mutate(Index=seq_len(nrow(.)))
...
.goslinOthers <- function(goslin.res){
    others <- goslin.res %>%
        dplyr::filter(
            stringr::str_starts(Lipid.Maps.Category, 'ST', negate=TRUE)) %>%
        dplyr::filter(
            stringr::str_detect(Normalized.Name, ' P-', negate=TRUE)) %>%
        tidyr::separate(
            Species.Name, into=c('class', 'Total.Chain.Sum'),
            sep=' ', remove=FALSE) %>%
        dplyr::mutate(
            class=ifelse(
...
.goslinFaChainLong <- function(merge.res){
    FA.chain.long <- merge.res %>%
        ...
        tidyr::separate(SN, into=c('CDB', 'OH'), sep=';', remove=FALSE,
                        fill='right', extra='merge') %>%
        tidyr::separate(CDB, into=c('C', 'DB'), sep=':') %>%
        tidyr::separate(
            OH, into=c('O1', 'O2'), sep=';', fill='right', extra='drop') %>%
...
.lionIdConversion <- function(goslin.char){
    ...
    sort.FA <- goslin.char2 %>%
        ...
        tidyr::gather(FA, chain, -seq_len(2)) %>%
