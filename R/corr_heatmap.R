      ...
      if(min(sample_corr_coef) >= 0 || max(sample_corr_coef) <= 0){
        if(plotly == TRUE){
          sample_hm <- iheatmapr::iheatmap(
            ...
            iheatmapr::add_col_dendro(col_dend, 
                                      side="top", 
                                      reorder=TRUE, size=0.1) %>%
            iheatmapr::add_row_dendro(col_dend, 
                                      side="right", reorder=TRUE, size=0.1)
          ...
          sample_hm <- stats::heatmap(sample_corr_coef, Rowv=T, Colv=T, 
        ...
        if(plotly == TRUE){
          ...
          sample_hm <- stats::heatmap(
            sample_corr_coef, Rowv=T, Colv=T, 
            dendrogram='both', trace="none", 
    ...
    if(sum(is.na(lipids_exp_data)) == 0 &
      ...
      if(min(lipids_corr_coef) > 0 | max(lipids_corr_coef) < 0){
        if(plotly == TRUE){
          ...
          lipids_hm <- stats::heatmap(
            lipids_corr_coef, Rowv=T, Colv=T, 
            dendrogram='both', trace="none", 
