#' @title DE_sub_char_2
#' @description In the \code{\link{DE_sub_char_2}}, lipid Characteristics Analysis will be conducted. Lipid species are categorized and summarised into a new lipid expression table according to two selected lipid characteristics, then conducted differential expressed analysis. \\
#' Note:
#' \enumerate{
#' \item The subgroup analysis is based on the first user-selected characteristics. So, use \code{\link{DE_char_2}} before using \code{\link{DE_sub_char_2}},
#' \item two selected characteristics should be both continuous data and one categorical data with one continuous data. The cut-offs of differentially expressed lipids are inputted by users.
#' }
...
#' @param lipid_char_table A data frame with lipid features, such as class, total length. NAs are allowed. The name of the first column must be "feature".
#' @param split_var A character string of the second lipid characteristic selected by users from \bold{lipid_char_table} for the subgroup analysis, such as class. \emph{NOTE: This parameter will be used to split data before entering the main lipid characteristic 'char_var'.}
...
#' @param trans_type Logical. If \bold{data_transform = TRUE}, transform exp_data by log10.
...
#' \enumerate{
#' \item result_table1: a data frame with scaled expression based on two user-selected lipid characters' grouping
#' \item result_table2: a data frame with two-way anova result with a post_hoc_test result
#' \item result_table3: a data frame of scaled expression based on the char_var's weighted mean
...
#' DE_sub_char_2(exp_data, data_transform=T, lipid_char_table = lipid_char_table,
#'               split_var = char_var[1], char_var = char_var[4], group_info = group_info,
#'               paired = F, sig_pvalue=0.05, sig_FC=2, exclude_var_missing=T,
#'               missing_pct_limit=50, replace_zero=T, zero2what='min', xmin=0.5,
#'               replace_NA=T, NA2what='min', ymin=0.5, pct_transform=T,
...
DE_sub_char_2 <- function(exp_data, data_transform=T, lipid_char_table, split_var, char_var,
  ...
  if("class" %in%colnames(lipid_char_table)){
    if(class(lipid_char_table[,'class'])!="character"){
  ...
  split_var_member <- lipid_char_table %>% dplyr::filter(!is.na(eval(parse(text = char_var)))) %>%
    .[[split_var]] %>% unique() %>% stats::na.omit()

  exp_data <- exp_data %>% dplyr::left_join(lipid_char_table[c('feature', split_var)], by='feature')
  ...
  for(var_member in 1:length(split_var_member)){
    split_exp_data <- exp_data %>% dplyr::filter(eval(parse(text = split_var))==split_var_member[var_member]) %>% dplyr::select(-split_var)
    split_lipid_char_table <- lipid_char_table %>% dplyr::filter(eval(parse(text = split_var))==split_var_member[var_member])
    ...
    exp_data_trans <- LipidSigR::data_process(spec2char, exclude_var_missing, missing_pct_limit,
    ...
    result_table1[[var_member]] <- result[[1]] %>% dplyr::select(split_var, dplyr::everything())
    result_table2[[var_member]] <- result[[2]] %>% dplyr::select(split_var, dplyr::everything())
    result_table3[[var_member]] <- result[[3]] %>% dplyr::select(split_var, dplyr::everything())
    result_table4[[var_member]] <- result[[4]] %>% dplyr::select(split_var, dplyr::everything())
