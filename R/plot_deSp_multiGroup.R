...
plot_deSp_multiGroup <- function(deSp_se){
    .check_inputSE(deSp_se, metadata_list=c(
        'all_deSp_result', 'sig_deSp_result', 'processed_abundance',
    ...
    ## Check results
    if(is.null(sig_deSp_result)){
        warning(paste("This case does not include any significant lipids;",
    ...
    if(nrow(all_deSp_result) > 0){
        ...
        message(paste("This case does not include any lipids for generating",
...
.lollipop_multiGroup <- function(deSp_result, significant=c('pval', 'padj')){
    ...
    n=t=NULL
    for(n in 1:2){
        for(t in 1:length(int.lolli$x$data[[n]]$text)){
            lipid <- NULL
...
.dot_multiGroup <- function(deSp_result, lipid_char,
    ...
    n=t=NULL
    for(n in 1:length(int.dot$x$data)){
        #int.dot$x$data[[n]]$marker$opacity <- 1
        for(t in 1:length(int.dot$x$data[[n]]$text)){
