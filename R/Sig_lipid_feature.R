...
#' @param lipid_char_table A data frame. NAs are allowed. The name of
#' first column must be "feature".A data frame with lipid features,
#' such as class, total length. NAs are allowed. The name of the
#' first column must be "feature".
...
#' \enumerate{
#' \item a bar chart shows the significant groups (values) with
#' mean fold change over 2 in the selected characteristics by colors
#' \item a lollipop plot to compare multiple values simultaneously and
#' it aligns the log2(fold change) of all significant groups(values) within
#' the selected characteristics
#' \item a word cloud of the count of each group(value) of the
...
#' exp_transform_non_log <- data_process(exp_data, exclude_var_missing=FALSE,
...
Sig_lipid_feature <- function(DE_species_table_sig, lipid_char_table,
                              char_var, sig_FC=2, plotly=TRUE){
  ...
  p.log2fc.sig <- ggplot2::ggplot(
    sig_class.log2FC,
    ggplot2::aes(x=stats::reorder(characteristic, -log2FC.mean),
                 y=log2FC.mean, fill=significant)) +
    ggplot2::geom_col() +
    ggplot2::geom_errorbar(ggplot2::aes(min=log2FC.mean, max=log2FC.meansd)) +
    ggthemes::theme_hc() +
    ggplot2::scale_fill_manual(breaks=c('NO', 'YES'),
                               values=c('#666666', '#FF6347'))+
    ...
    ggplot2::theme(plot.title=ggplot2::element_text(size=16, hjust=0.5),
          ...
          axis.text.x=ggplot2::element_text(angle=90, vjust=0.5,
                                            hjust=1, size=12),#, angle=30),
          axis.text.y=ggplot2::element_text(size=12))
  in.log2fc.sig <- p.log2fc.sig
  if(plotly == TRUE){
    ...
    for(i in seq_len(length(in.log2fc.sig.ggplotly$x$data))){
      in.log2fc.sig.ggplotly$x$data[[i]]$text <-
        gsub('(characteristic, -log2FC.mean)', '',
             in.log2fc.sig.ggplotly$x$data[[i]]$text)
      in.log2fc.sig.ggplotly$x$data[[i]]$text <-
        gsub('reorder', paste0(char_var,' :'),
             in.log2fc.sig.ggplotly$x$data[[i]]$text)
      in.log2fc.sig.ggplotly$x$data[[i]]$text <-
        gsub('\\(\\): ', '',
             in.log2fc.sig.ggplotly$x$data[[i]]$text)
      in.log2fc.sig.ggplotly$x$data[[i]]$text <-
        gsub('log2FC.mean: ', 'mean(log2FC) :',
             in.log2fc.sig.ggplotly$x$data[[i]]$text)
      in.log2fc.sig.ggplotly$x$data[[i]]$text <-
  ...
  p.sig.dotchart <- ggpubr::ggdotchart(sig.dotchart, combine=TRUE,
    ...
    ggplot2::geom_point(
      ggplot2::aes(text=paste("Characteristic :", characteristic, "<br>",
                              "log2FC : ", round(log2FC,2)),
                   color=characteristic,size=3)) +
    ggplot2::guides(size="none")
  in.sig.dotchart <- p.sig.dotchart
  if(plotly == TRUE){
  ...
  if(plotly == TRUE){
    ...
    wordcloud::wordcloud(wc.tab$characteristic, wc.tab$freqs,
                               min.freq = 1, random.order = FALSE,
                               ordered.colors = FALSE,
                               colors = grDevices::rainbow(nrow(wc.tab)))
    wc <- grDevices::recordPlot()
    grDevices::dev.off()
  }
