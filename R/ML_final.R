#' @title ML_final
#' @description This function constructs the machine learning model based on
#' the output data processed by \code{\link{ML_data_process}}.
#' @param ML_data The output data frame(list[[2]]) of
#' \code{\link{ML_data_process}}.
#' @param ranking_method A character string for the ranking method to be
#' computed. Allowed methods include 'p_value', 'pvalue_FC', 'ROC',
#' 'Random_forest', 'SVM', 'Lasso', 'Ridge', 'ElasticNet'.
#' @param ML_method A character string for the machine learning method to be
#' computed. Allowed methods include 'Random_forest', 'SVM', 'Lasso', 'Ridge',
#' 'ElasticNet', 'xgboost'.
#' @param split_prop A numeric value between 0.1 to 0.5 indicates the
#' proportion of data to be retained for modeling/analysis.
#' @param nfold A numeric value. The original dataset is randomly partitioned
#' into N fold equal size subsamples.
#' @param alpha (optional) A numeric value between 0 and 1. Only need when
#' selecting 'ElasticNet' as ML_method. 0 for Ridge and 1 for Lasso.
#' @return Return a list of 6 data frames and 3 lists.
#' \enumerate{
#' \item cv_model_result: data frame, an machine learning model
#' after cross-validation.
#' \item cv_cm_result: data frame, confusion matrix after cross-validation.
#' \item cv_ROC_result: data frame, receiver operating characteristic after
#' cross-validation.
#' \item cv_PR_result: data frame, Precision-Recall after cross-validation.
#' \item cv_meanROC_result: data frame, mean receiver operating characteristic
#' of cross-validation.
#' \item cv_feature_save: list of length 10, feature after cross-validation.
#' \item cv_varimp_result: data frame, variable importance after
#' cross-validation.
...
ML_final <- function(ML_data,ranking_method, ML_method,
  ...
  if(!ranking_method %in% c('p_value', 'pvalue_FC', 'ROC', 'Random_forest',
                            'SVM', 'Lasso', 'Ridge', 'ElasticNet')){
    stop('ranking_method must be one of the strings "p_value", "pvalue_FC",
         "ROC", "Random_forest", "SVM", "Lasso", "Ridge", or "ElasticNet".')
  }
  if(!ML_method %in% c('Random_forest', 'SVM', 'Lasso', 'Ridge',
                       'ElasticNet', 'xgboost')){
    stop('ML_method must be one of the strings "Random_forest", "SVM", "Lasso",
         "Ridge", "ElasticNet", or "xgboost".')
  }
  ...
  feature_selection <- function(data, ranking_method, topN){
    ...
    if(ranking_method == 'p_value'){
      result <- data %>%
        ...
        dplyr::mutate(p_value=tryCatch(stats::t.test(unlist(group0),
                                                     unlist(group1),
      ...
      result <- result %>%
        dplyr::arrange(p_value)  %>% .[seq_len(topN),1] %>% unlist()
      return(result)
      ...
      result <- data %>%
        ...
        tidyr::spread(group, value) %>%
        dplyr::group_by(feature) %>%
        dplyr::mutate(pvalue_FC=-log10(tryCatch(stats::t.test(unlist(group0),
                                                              unlist(group1),
                                                              var.equal=
                                                                TRUE)$p.value,
                                           error=function(e){NA}))*abs(
                                             mean(unlist(group0),
                                                  na.rm=TRUE)/mean(unlist(
                                                    group1), na.rm=TRUE))) %>%
        dplyr::select(feature, pvalue_FC)


      result <- result %>%
        dplyr::arrange(dplyr::desc(pvalue_FC)) %>%
      ...
      result <- data[-1] %>%
        ...
        dplyr::group_by(feature) %>%
        dplyr::mutate(auc=tryCatch(as.numeric(pROC::roc(unlist(group),
                                                        unlist(value),
                                                        quiet=TRUE)$auc),
                              error=function(e){NA})) %>%
      ...
      model <- ranger::ranger(group ~ ., data = data, num.trees = 500,
      ...
      model <- e1071::svm(group~., kernel='linear',
                          data=data, type='C-classification', probability=TRUE)
      result <- stats::coef(model)[-1] %>%
        abs() %>% sort(decreasing = TRUE) %>% names()
      return(result[seq_len(topN)])
      ...
      coef <- stats::coef(model,s = 'lambda.min') %>% as.matrix() %>%
  ...
  ML_main <- function(train_data, test_data, ML_method){
    ...
    if(ML_method == 'Random_forest'){
      ...
      model_result <- data.frame(ML_method=ML_method,
                                 ID=rownames(test_data),
      ...
      model_result <- data.frame(ML_method=ML_method, ID=rownames(test_data),
                                 true_label=test_data[[1]],
                                 pred_prob=pred_prob) %>%
        dplyr::mutate(pred_label=ifelse(pred_prob > 0.5,1,0))
      ...
      pred_prob <- stats::predict(model, s='lambda.min',
                                  newx=as.matrix(test_data[-1]),
                                  type="response")[,1]
      ...
      model <- xgboost::xgboost(data=as.matrix(train_data[-1]),
                                abel=train_data[[1]], nrounds=cv_nround_min,
                                max_depth=3,
                                objective="binary:logistic",
                                verbose=0)
    ...
    cm <- caret::confusionMatrix(as.factor(model_result$true_label),
                                 factor(model_result$pred_label,
                                        levels=c(0, 1)),positive='1')
    cm_result <-rbind(data.frame(index=names(cm$overall)[1],
                                 value=cm$overall[1]),
    ...
    ROC_plot <- pROC::roc(model_result$true_label,
                          model_result$pred_prob,
                          quiet=TRUE)
    thershold <- c(seq(0, 1, length.out=n_ROC))

    meanROC <- pROC::coords(ROC_plot, thershold,
    ...
    return(list(model_result, cm_result, ROC_result, PR_result,
                meanROC_result, varImp_result, model))
  }
  ...
  ML <- function(data, ranking_method, ML_method, split_prop){
    ...
    for(a in seq_len(nfold)){
      ...
      for(b in seq_len(length(sele_feature_num))){
        sele_feature <- feature_selection(train_data, ranking_method,
                                          sele_feature_num[b])
        ...
        ML_result <- ML_main(train_data[feature_loc],
                             test_data[feature_loc], ML_method)
    ...
    cv_model_result <- cv_model_result %>%
      dplyr::mutate(ranking_method=ranking_method) %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything()) %>%
      dplyr::filter(!is.na(ML_method))
    cv_cm_result <- cv_cm_result %>%
      dplyr::mutate(ranking_method=ranking_method) %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything()) %>%
      dplyr::filter(!is.na(ML_method))

    cv_ROC_result <- cv_ROC_result %>%
      dplyr::mutate(ranking_method=ranking_method) %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything()) %>%
      dplyr::filter(!is.na(ML_method))

    cv_PR_result <- cv_PR_result %>%
      dplyr::mutate(ranking_method=ranking_method) %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything()) %>%
      dplyr::filter(!is.na(ML_method))
    ...
    cv_meanROC_result <- cv_meanROC_result %>%
      ...
      dplyr::mutate(ranking_method=ranking_method,
                    ML_method=ML_method, cv_fold='mean') %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything())

    cv_varimp_result <- cv_varimp_result %>%
      dplyr::mutate(ranking_method=ranking_method) %>%
      dplyr::select(ranking_method, ML_method, cv_fold,
                    feature_num, dplyr::everything()) %>%
      dplyr::filter(!is.na(ML_method))
