...
ML_final <- function(ML_data,ranking_method, ML_method,
  ...
  feature_selection <- function(data, ranking_method, topN){
    ...
    if(ranking_method=='p_value'){
      result <- data %>% dplyr::mutate(group=ifelse(group==0,'group0', 'group1')) %>%
        ...
        dplyr::mutate(p_value = tryCatch(stats::t.test(unlist(group0), unlist(group1), var.equal = T)$p.value,
                                  error=function(e){NA})) %>%
        dplyr::select(feature, p_value)

      result <- result %>% dplyr::arrange(p_value)  %>% .[1:topN,1] %>% unlist()
      return(result)
      ...
      result <- data %>% dplyr::mutate(group=ifelse(group==0,'group0', 'group1')) %>%
        ...
        tidyr::spread(group, value) %>%
        dplyr::group_by(feature) %>%
        dplyr::mutate(pvalue_FC = -log10(tryCatch(stats::t.test(unlist(group0), unlist(group1), var.equal = T)$p.value,
                                           error=function(e){NA}))*abs(mean(unlist(group0),na.rm=T)/mean(unlist(group1), na.rm=T))) %>%
        dplyr::select(feature, pvalue_FC)


      result <- result %>% dplyr::arrange(dplyr::desc(pvalue_FC)) %>% .[1:topN,1] %>% unlist()
      ...
      result <- data[-1] %>%
        ...
        dplyr::group_by(feature) %>%
        dplyr::mutate(auc = tryCatch(as.numeric(pROC::roc(unlist(group), unlist(value),quiet=T)$auc),
                              error=function(e){NA})) %>%
        dplyr::select(feature, auc) %>%
        dplyr::arrange(dplyr::desc(auc)) %>% .[1:topN,1,drop=T]

      return(result)

    }else if(ranking_method=='Random_forest'){
      model <- ranger::ranger(group ~ ., data = data, num.trees = 500, importance='impurity', probability = T)
      ...
      result <- stats::coef(model)[-1] %>% abs() %>% sort(decreasing = T) %>% names()
      return(result[1:topN])
      ...
      coef <- stats::coef(model,s = 'lambda.min') %>% as.matrix() %>% as.data.frame()%>% .[-1, ,drop=F]
  ...
  ML_main <- function(train_data, test_data, ML_method){
    ...
    if(ML_method=='Random_forest'){
      model <- ranger::ranger(group ~ ., data = train_data, num.trees = 500, importance='impurity', probability = F)
      ...
      model <- e1071::svm(group~., kernel='linear', data=train_data, type='C-classification', probability=T)

      pred_prob <- stats::predict(model, test_data[-1], probability = T) %>% attributes() %>%
        .$probabilities %>% .[,2]
      ...
      varimp <- stats::coef(model,s = 'lambda.min') %>% as.matrix() %>% as.data.frame()%>% .[-1, ,drop=F]
    ...
    ROC_plot <- pROC::roc(model_result$true_label, model_result$pred_prob, quiet = T)
  ...
  ML <- function(data, ranking_method, ML_method, split_prop){
    ...
    for(a in 1:nfold){
      print(stringr::str_c('CV fold ', as.character(a), ' done'))
    ...
    cv_meanROC_result <- cv_meanROC_result %>%
      ...
      dplyr::summarise(sensitivity=mean(sensitivity,na.rm=T),
