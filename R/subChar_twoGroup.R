...
subChar_twoGroup <- function(
    ...
    if(is.null(char) | isFALSE(char %in% c('Total.C', 'Total.DB', 'Total.OH',
                                           'FA.C', 'FA.DB', 'FA.OH'))){
        stop("The 'char' parameter must be a numeric lipid characteristic,
             such as 'Total.C'. Please execute list_lipid_char() to view the
             valid lipid characteristics you can input.")
    }
    if(char == subChar){
        stop("The 'char' and 'subChar' parameters cannot be the same.")
    }
    if(is.null(subChar) | isFALSE(subChar %in% char_list)){
        stop("The 'subChar' parameter is not in the list of lipid
