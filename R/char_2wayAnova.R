...
#' @description This function calculates two-way ANOVA for all lipid characteristics.
...
#' @param ratio_transform Character. Method for transform the ratio-based abundance.
...
char_2wayAnova <- function(processed_se, ratio_transform=c('none', 'log2'),
    ...
    if(is.null(ratio_transform) | isFALSE(ratio_transform %in% c('none', 'log2'))){
        stop("The 'ratio_transform' parameter must be either 'none' or 'log2'.")
    }
    if(is.null(char_transform) | isFALSE(char_transform %in% c('none', 'log10', 'square', 'cube'))){
        stop("The 'char_transform' parameter must be one of 'none', 'log10', 'square' or 'cube'.")
    }
    ## convert_sp2char
    ...
    ## bind 2 SE
    SummarizedExperiment::assayNames(char_se) <- SummarizedExperiment::assayNames(ratio_se)
