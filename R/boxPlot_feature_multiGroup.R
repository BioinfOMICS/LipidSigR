...
#' @description This function plots an abundance box plot between multiple groups for a user-selected feature (lipid).
...
#' @param feature Character. A feature (lipid) for plotting. It must be one of the lipids in the lipid abundance data.
...
#' @param post_hoc_sig Character. The p-value to be used for to determine significance, which is only
#' applicable for the post hoc test after the Kruskal-Wallis test. Must be one
#' of "padj" (adjusted p-value) or "pval" (p-value). Default is \code{'pval'}.
...
boxPlot_feature_multiGroup <- function(
    ...
    if(test == 'One-way ANOVA' & post_hoc_sig == 'pval'){
        post_hoc_sig <- 'padj'
        warning(paste("post_hoc_sig is only for the post hoc test after the",
    ...
    if(!feature %in% abundance$feature) stop("'feature' must be one of the lipids in your lipid abundance data; please revise your input.")
    ## Group information
    group_info <- .extract_df(processed_se, type="group")
    group_type <- .check_nGroup(group_info)
    if(group_type != 'multiple') stop('This function is used for multiple groups. If your data consists of two groups, please use boxPlot_feature_twoGroup() for analysis.')
    group_info %<>%
        dplyr::mutate(
    ...
    if(!all(is.na(comp_res$res_table$pval))){
        ...
        if(transform == 'log10'){
            ...
            Y <- paste0(stringr::str_to_title(transform), '-transformed abundance')
        }else{
        ...
        return(warning(paste(
            'Unable to perform statistical tests for this feature; therefore,',
