...
#' @description This function plots an abundance box plot between two groups for a user-selected feature (lipid).
...
boxPlot_feature_twoGroup <- function(
    ...
    if(group_type != 'two') stop('This function is used for two groups. If your data consists of multiple groups, please use "boxPlot_feature_multiGroup" for analysis.')
    if(sum(group_info$group == 'ctrl') < 2 | sum(group_info$group == 'exp') < 2 ){
...
.box_stat_twoGroup <- function(sub_abund, group_info, ref_group, paired_sample, test){
    rstatix.ref.group <- setdiff(unique(group_info$original_group_name), ref_group)
    if(test == 't-test'){
        test_res <- tryCatch({
