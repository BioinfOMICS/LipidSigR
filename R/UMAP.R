...
#' @param group_info A data frame for marking clustering sample on PCA chart.
#' It is according to
#' \bold{Group information table}(after convert to ctrl and exp).
...
#'   \item \bold{"Z"} or \bold{"scale"} or \bold{TRUE}: Scale each column to
#'   zero mean and variance 1.
#'   \item \bold{"maxabs"}: Center each column to mean 0, then divide each
#'   element by the maximum absolute value over the entire matrix.
#'   \item \bold{"range"}: Range scale the entire matrix, so the smallest
...
#' One of \bold{"euclidean"}, \bold{"cosine"}, \bold{"manhattan"},
#' \bold{"hamming"}, \bold{"categorical"}.
#' @param cluster_method A character string indicating which method to be
#' used for clustering. Allowed method include \bold{"kmeans"},
#' \bold{"kmedoids"}, \bold{"hclustering"}, \bold{"dbscan"},
#' \bold{"group_info"}.
...
UMAP <- function(exp_transform_table, group_info=NULL, sig_feature=NULL,
    ...
    if(ncol(group_info) == 4){
      ...
      if(sum(group_info[, 1] %in%
        ...
        stop("group_info 'sample_name' must same as the name of
             samples of exp_transform_table")
      }
      ...
      if(!is.null(insert_ref_group)){
        if(!insert_ref_group %in% group_info[, 3]){
          stop("The reference group entered by users must be included in the
               group_info.")
        }
      }
  ...
  if(cluster_method == 'kmeans'){
    ...
    cluster_group <- cluster::pam(umap_result, k=group_num,
    ...
    group_order <- purrr::map_dbl(rownames(umap_table),
                                  function(x){which(
    ...
    if(plotly == TRUE){
      ...
      for (i in seq_len(length(umap_plot$x$data))){
        if (!is.null(umap_plot$x$data[[i]]$name)){
          umap_plot$x$data[[i]]$name <- gsub(
            "\\(", "",
            stringr::str_split(umap_plot$x$data[[i]]$name, ",")[[1]][1])
        }
        if(i <= length(unique(cluster_group))){
          umap_plot$x$data[[i]]$text <- paste(
            "x :", round(umap_plot$x$data[[i]]$x, 3),
            "\ny :", round(umap_plot$x$data[[i]]$y, 3),
            "\nGroups :", umap_plot$x$data[[i]]$name,
            "\nSample name :",umap_data[which(
              umap_data$group == umap_plot$x$data[[i]]$name), ]$sample_name)
        }
      }
    }
    ...
    if(plotly == TRUE){
      ...
      for (i in seq_len(length(umap_plot$x$data))){
        ...
        if(i <= group_num){
          ...
          umap_plot$x$data[[i]]$text <- paste("Groups :",
                                              umap_plot$x$data[[i]]$name)
        }
      }
