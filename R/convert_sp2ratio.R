...
convert_sp2ratio <- function(processed_se, transform=c('none', 'log2')){
    ...
    ether.ester <- odd.even <- LPL <- LPE <- LPS <-
        LPG <- LPI <- LPA <- LPC <- PC.PE <- TG.DG <-NULL
    ## Extract SE
    ...
    if(all(is.null(ether.ester), is.null(odd.even), is.null(LPL), is.null(LPE),
        ...
        warning("There are no ratio characteristics that can be converted in your dataset.")
        return(NULL)
    }
    ...
    message('There are ', length(char.list), ' ratio characteristics ',
...
.etherEsterRatio <- function(expr.mat){
    ...
    ether.class <- expr.mat$feature[
        which(stringr::str_detect(expr.mat$feature, ' O-| P-'))]
    # Detect ester lipid class
    ester.class <- expr.mat$feature[
        which(expr.mat$feature %in%
                  stringr::str_remove(ether.class, ' O-| P-'))]
    # Intersection
    pair.class <- intersect(
        ester.class, stringr::str_remove(ether.class, ' O-| P-'))
    ...
    reshape.expr <- expr.mat %>%
        ...
        dplyr::mutate(class=stringr::str_remove(feature, ' O-| P-'),
                      group=ifelse(stringr::str_detect(feature, ' O-| P-'),
                                   'ether', 'ester')) %>%
...
.oddEvenRatio <- function(speciesAbund, speciesChar){
    ...
    char.mat <- speciesAbund %>%
        ...
        tidyr::gather(sample_name, exp, -seq_len(4)) %>%
...
.twoClassRatio <- function(
        expr.mat, riskClass, refClass, aspect=c('LPL', 'others')){
    sub.mat <- expr.mat %>%
    ...
    reshape.expr <- sub.mat %>%
        ...
        dplyr::select(sample_name, 'risk'=all_of(riskClass),
                      'ref'=all_of(refClass)) %>%
...
.lysoGpRatio <- function(speciesAbund, speciesChar){
    ...
    char.mat <- speciesAbund %>%
        ...
        tidyr::gather(sample_name, exp, -seq_len(2)) %>%
