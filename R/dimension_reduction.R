...
dr_pca <- function(
        processed_se, scaling=TRUE, centering=TRUE, clustering=c('kmeans', 'kmedoids', 'hclustering', 'dbscan', 'group_info'),
    ...
    if(!is.numeric(plot_topN) | isFALSE(.check_numeric_range(plot_topN, 1, 10))){
        stop('plot_topN must be a numeric value between 1 and 10.')
    }

    abundance <- .extract_df(processed_se, type = "abundance")
    group_info_raw <- .extract_df(processed_se, type = "group")

    if(!is.null(S4Vectors::metadata(processed_se)[["sig_deChar_result"]])) {
        sig_table <- S4Vectors::metadata(processed_se)[["sig_deChar_result"]]
    } else {
        sig_table <- S4Vectors::metadata(processed_se)[["sig_deSp_result"]]
    }
    if (is.null(group_info_raw$group)) {
        group_info <- NULL
    } else {
...
dr_tsne <- function(
        processed_se, pca=TRUE, perplexity=5, max_iter=500, clustering=c('kmeans', 'kmedoids', 'hclustering', 'dbscan', 'group_info'),
        cluster_num=2, kmedoids_metric=NULL, distfun=NULL, hclustfun=NULL, eps=NULL, minPts=NULL){
...
dr_umap <- function(
    ...
    if(!is.numeric(n_neighbors) | isFALSE(.check_numeric_range(n_neighbors, 1, NULL))){
        stop('n_neighbors must be greater than 1.')
    }
    if(is.null(scaling) | isFALSE(scaling %in% c('none', FALSE, NULL, 'Z', 'scale', TRUE, 'maxabs', 'range', 'colrange'))){
        stop('scaling must be one of "none", FALSE, NULL, "Z", "scale", TRUE, "maxabs", "range", or "colrange".')
    }
    if (is.null(umap_metric) | isFALSE(umap_metric %in% c('euclidean', 'cosine', 'manhattan', 'hamming', 'categorical')) ) {
        stop("umap_metric must be one of 'euclidean', 'cosine', 'manhattan', 'hamming', or 'categorical'.")
    }
    if (is.null(clustering) | isFALSE(clustering %in% c('kmeans', 'kmedoids', 'hclustering', 'dbscan', 'group_info')) ) {
        stop("clustering must be one of 'kmeans', 'kmedoids', 'hclustering', 'dbscan', or 'group_info'.")
    }
...
#'     cluster_num=2, kmedoids_metric=NULL, distfun=NULL, hclustfun=NULL, eps=NULL, minPts=NULL)
dr_plsda <- function(
    processed_se, ncomp=2, scaling=TRUE,
    clustering=c('kmeans', 'kmedoids', 'hclustering', 'dbscan', 'group_info'),
    cluster_num=2, kmedoids_metric=NULL, distfun=NULL, hclustfun=NULL, eps=NULL, minPts=NULL){
    ...
    plsda_table <- abundance[c(colnames(SummarizedExperiment::rowData(processed_se))[[1]], group_info$sample_name)] %>%
        dplyr::select(-1) %>% t()
