...
#' @param feature_n A numeric value specifying the number of features
#' to be shown.
...
SHAP <- function(data, best_model, best_model_feature,
                 ML_method ,feature_n, nsim,plotly=TRUE){

  data <- as.data.frame(data)
  if(!ML_method %in% c('Random_forest','SVM','Lasso','Ridge',
                       'ElasticNet','xgboost')){
    stop('ML_method must be one of the strings "Random_forest",
         "SVM", "Lasso", "Ridge", "ElasticNet", or "xgboost".')
  }
  if(!class(feature_n) %in% c("numeric", "integer")){
    ...
    if(!feature_n %in% names(best_model)){
      stop(paste0('feature_n must be one of ',
                  paste(unique(data2$feature_num), collapse=", ")))
    }
  }
  ...
  if(ML_method == 'xgboost'){
    ...
    pred <- function(object, newdata) {
      attributes(stats::predict(object,
                                newdata=newdata,
  ...
  shap_long <- shap_score %>%
    ...
    dplyr::mutate(ID=rep(seq_len(nrow(shap_score)), ncol(shap_score)),
                  rfvalue=unlist(data[-1])) %>%
  ...
  if(plotly == TRUE){
    mean_shapley_plot <- shap_long[,c(2,6)] %>%
      as.data.frame %>%
      unique() %>%
      dplyr::arrange(dplyr::desc(mean_value)) %>% .[seq_len(topN),] %>%
      plotly::plot_ly(x=~mean_value,
                      y=~stats::reorder(variable,mean_value),
                      type="bar",
                      orientation="h",
                      hoverinfo="text",
                      marker=list(color=~-mean_value, colorscale="Blues"),
                      text=~paste("Feature :", variable,
                                  "<br>Mean shapley value :",
                                  round(mean_value, 2))) %>%
      plotly::layout(title="SHAP feature importance",
                     xaxis=list(title="mean(|Shapley value|)",
                                nticks=15, showline=TRUE, mirror='all'),
                     yaxis=list(title=" "))
  }else{
    mean_shapley_plot <- shap_long[,c(2,6)] %>%
      as.data.frame %>%
      unique() %>%
      dplyr::arrange(dplyr::desc(mean_value)) %>% .[seq_len(topN),] %>%
      ggplot2::ggplot(ggplot2::aes(y=mean_value,
               x=stats::reorder(variable, mean_value),
               fill=-mean_value)) +
      ...
      ggplot2::theme_bw();mean_shapley_plot
  }


  all_shapley_plot <- SHAPforxgboost::shap.plot.summary.wrap2(
    shap_score=shap_score, X=as.matrix(data[-1]), top_n=topN)
  if(plotly == TRUE){
    all_shapley_plot <- plotly::ggplotly(all_shapley_plot)
  }
  shap_long <- as.data.frame(shap_long)
  colnames(shap_long)[c(3,4,5,6)] <- c('shapley_value',
                                       'raw_value',
                                       'normalized_value',
                                       'mean_shapley_value')
