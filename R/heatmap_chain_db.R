...
heatmap_chain_db <- function(
    ...
    if(n_group == 'two'
       & any((is.null(test) | !test %in% c('t-test', 'Wilcoxon test'))) ){
        stop("'One-way ANOVA' and 'Kruskal-Wallis test' are for multiple",
    ...
    if(n_group == 'multiple'
       & (is.null(test) | !test %in% c('One-way ANOVA', 'Kruskal-Wallis test'))){
        stop("'t-test' and 'Wilcoxon test' are for two group comparisons.",
    ...
    if(is.null(total_chain)){
        message("This lipid characteristic or characteristic feature does",
...
.chain_db_abundance <- function(lipid_char, abundance, group_info, char_feature){
    ...
    char.abund <- lipid_char %>%
        ...
        tidyr::gather(sample_name, abundance, -seq_len(5)) %>%
