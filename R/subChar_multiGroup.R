...
subChar_multiGroup <- function(
    ...
    if(is.null(char) | isFALSE(char %in% c('Total.C', 'Total.DB', 'Total.OH',
        ...
        stop(paste("The 'char' parameter must be a numeric lipid ",
                   ...
                   "list_lipid_char() to view the valid lipid characteristics",
                   "you can input."))
    }
    if(char == subChar){
    ...
    if(group_type != 'multiple') stop('This function is used for multiple groups. If your data consists of two groups, please use subChar_twoGroup() for analysis.')
    group_info %<>%
        dplyr::mutate(
