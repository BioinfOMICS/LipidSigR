...
#' @description This function shows the significant lipid species based on different
#' lipid characteristics and visualizes the difference between control and experimental groups.
...
#' \enumerate{
#' \item interactive_barPlot & static_barPlot: a bar plot distinguishes significant groups (values) exhibiting a mean fold change greater than 1.
#' (NOTE: The bar chart is not generate for multiple-group data.)
#' \item interacitve_lollipop & static_lollipop: a lollipop plot compares all significant groups
#' within the selected characteristic by log2(fold change) for two-group data and -log10(p-value) for multiple-group data.
#' \item interactive_wordCloud & static_wordCloud: a word cloud visualizes the frequency of each group (value) associated with the chosen characteristic.
...
char_association <- function(deSp_se, char){
    ...
    .check_inputSE(
        deSp_se,
        metadata_list=c("all_deSp_result", "sig_deSp_result", "significant", "p_cutoff"))
    ...
    if(n_group == 'two'){
        ...
        interactive_barPlot <- "Only provided when two-group differential analysis data is input."
        static_barPlot <- "Only provided when two-group differential analysis data is input."
        table_barPlot <- "Only provided when two-group differential analysis data is input."
        # Lollipop plot
    ...
    return(list(
        interactive_barPlot=interactive_barPlot, interacitve_lollipop=lolli.res$lolli.plotly,
...
.sigDeSpeciesBar <- function(plot.tab, char, FC_cutoff){
    ...
    bar.tab <- plot.tab %>% dplyr::group_by(characteristic) %>%
        ...
        dplyr::mutate(significant = ifelse(abs(log2FC.mean) > log2(FC_cutoff), 'Yes', 'No'),
               ...
               hover = paste0(
                   ...
                   '<br />Mean of log2FC: ', format(log2FC.mean, digits=3, scientific=TRUE),
    ...
    # Bar plot
    bar <- ggplot2::ggplot(bar.tab, ggplot2::aes(x=x, y=log2FC.mean, fill=significant)) +
        ggplot2::geom_col() +
        ggplot2::geom_errorbar(ggplot2::aes(min=log2FC.mean, max=log2FC.mean+(log2FC.sd*log2FC.direction))) +
        ggthemes::theme_hc() +
        ggplot2::scale_fill_manual(breaks=c('Yes', 'No'), values=c('#FF4500', '#666666')) +
        ggplot2::labs(x='', y='log2(Fold Change)', fill=paste0('> ', FC_cutoff, ' FC')) +
...
.sigDeSpeciesLollipopTwoGroups <- function(plot.tab, char){
    ...
    for(t in 1:length(lollipop.plotly$x$data)){
        if(t == 1){
            lollipop.plotly$x$data[[t]]$text <- NULL
        }else{
            for(l in 1:length(lollipop.plotly$x$data[[t]]$text)){
