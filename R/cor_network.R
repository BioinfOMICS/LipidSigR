...
cor_network <- function(exp_transform_table, lipid_char_table,
  ...
  if(class(exp_transform_table[,1])!="character"){
    stop("exp_transform_table first column must contain a list of lipids names (features)")
  }
  if(ncol(exp_transform_table)==2){
    if(class(exp_transform_table[,1])!="character" | sum(class(exp_transform_table[,-1])%in%c("numeric","integer"))!=1){
      stop("exp_transform_table first column type must be 'character',others must be 'numeric'")
    }
  }else{
    if(class(exp_transform_table[,1])!="character" | sum(sapply(exp_transform_table[,-1], class)%in%c("numeric","integer"))!=ncol(exp_transform_table[,-1])){
      stop("exp_transform_table first column type must be 'character',others must be 'numeric'")
    }
  }
  ...
  visNet <- visNetwork::visNetwork(node_table, edge_table) %>%
    ...
    visNetwork::visLegend(useGroups = F,addNodes=ledges,width = 0.15)
