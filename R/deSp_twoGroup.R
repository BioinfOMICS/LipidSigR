...
deSp_twoGroup <- function(
    ...
    if (.check_nGroup(group_info_raw) != 'two') {
        stop('This function is used for two groups. If your data consists of multi groups, please use deSp_multiGroup() for analysis.')
    }
    ...
    deSp_se <- SummarizedExperiment::SummarizedExperiment(
        ...
        metadata=list(all_deSp_result=diff_table$all_table,
                      sig_deSp_result=diff_table$sig_table,
...
plot_deSp_twoGroup <- function(deSp_se){
    .check_inputSE(
        deSp_se,
        metadata_list=c("all_deSp_result", "sig_deSp_result", "significant", "p_cutoff", "FC_cutoff"))
    ...
    if (isTRUE("sig_FC" %in% colnames(all_table)) ){
        ...
        lipidPlot <- lipidPlot +
            ggplot2::geom_point(
                ggplot2::aes(
                    ...
                    round(log2FC, 2), "<br>", "-log10(p-value) :",
