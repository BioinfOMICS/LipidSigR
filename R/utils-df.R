...
#' }
#' @param n_group Character. 'two' for inputting two-group data and
...
as_summarized_experiment <- function(
        abundance, goslin_annotation, group_info=NULL, n_group=c('two', 'multiple'),
        paired_sample=FALSE){
    ## check input data --------------
    .check_valid_inputData(
        abundance, goslin_annotation, group_info, n_group, paired_sample)
    ...
    if (!is.null(group_info)) {
        ...
        .check_group_info(abundance, group_info, n_group, paired_sample)
    }
    ## check abundance data
    abundance_processed <- .check_abundance(abundance)
    ...
    SE_data <- SummarizedExperiment::SummarizedExperiment(
        ...
        colData=seCol)
    ## summary
    message(.inputData_info(SE_data, group_info, paired_sample))
    return(SE_data)
}

.inputData_info <- function(se, group_info, paired_sample) {
