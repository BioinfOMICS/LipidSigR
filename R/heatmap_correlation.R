...
heatmap_correlation <- function(
    ...
    if(type == 'sample'){
        ...
        if(all(!is.na(corr_coef)) & nrow(corr_coef)>=2 & ncol(corr_coef)>=2){
            ...
            static_plot <- .static_heatmap(corr_coef, sample_abundance, correlation, distfun, hclustfun)
        } else {
        ...
        if(all(!is.na(lipids_abundance)) & nrow(lipids_abundance)>=2 & ncol(lipids_abundance)>=2){
            ...
            static_plot <- .static_heatmap(corr_coef, lipids_abundance, correlation, distfun, hclustfun)
        } else {
            stop("Not enough data for plotting the heatmap.")
        }
    }
...
.static_heatmap <- function(corr_coef, abundance, correlation, distfun, hclustfun){

    col_dend <- if(distfun %in% c("pearson", "kendall", "spearman")) {
        stats::hclust(
            stats::as.dist(
                1 - stats::cor(abundance, method=distfun)), method=hclustfun)
    } else {
        stats::hclust(
            stats::dist(t(abundance), method=distfun), method=hclustfun)
    }
