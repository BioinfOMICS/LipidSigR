#' @title ROC_plot_all
#' @description The receiver operating characteristic (ROC) curve is one of 
...
#' }
#' @param data1 receiver operating characteristic after cross-validation. An 
...
#' @param feature_n A numeric value specifying the number of features to be 
#' shown.
...
ROC_plot_all <- function(data1, data2, feature_n, plotly=TRUE){
  ...
  if(plotly == TRUE){
    mean_AUC_plot <- plotly::plot_ly(mean_AUC_plot, 
                                     x=~(1-specificity), 
                                     y=~sensitivity)%>%
      plotly::add_lines(name=~reorder(feature_num_label, 
                                      as.numeric(feature_num)), 
                        hoverinfo="text", 
                        text=~paste("feature_num :", feature_num, 
                                    "<br>", feature_num_label1)) %>%
      plotly::add_lines(x=0:1, y=0:1, 
  ...
  if(plotly == TRUE){
    ...
    ROC_plot <- plotly::plot_ly(ROC_plot_1, 
                                x=~(1-specificity), 
                                y=~sensitivity, hoverinfo=NULL)%>%
      plotly::add_trace(data=ROC_plot_1, color=~cv_fold, colors ="gray", 
                        hoverinfo="text", mode="lines", type='scatter', 
                        text=~paste("cv_fold :", cv_fold))%>%
      plotly::add_trace(data=ROC_plot_2, name="mean", 
      ...
      plotly::add_lines(x=0:1, y=0:1, 
                        ...
                        showlegend=FALSE) %>%
      plotly::layout(xaxis=list(title="1-specificity"),
                     legend=list(title=list(text="cv_fold"), 
                                 y=0.5, x=1.1, font=list(size=9)),
                     title =list(size=15, y=0.99, x=0.1,
                                 text=stringr::str_c('ROC curve for ', 
    ...
    color_break <- unique(ROC_plot$cv_fold)
    color <- ifelse(color_break=='mean','red','gray')
    ROC_plot <- ggplot2::ggplot(ROC_plot,
                               ggplot2::aes(x=(1-specificity),
                                            y=sensitivity,
                                            color=cv_fold)) +
      ggplot2::geom_line() +
      annotate("text", x=0.6, y=0.15, 
               label=stringr::str_c('AUC=', as.character(mean_AUC), '(', 
                                    as.character(lower95), '-', 
                                    as.character(upper95), 
                                    ')\n', 'pvalue=', 
                                    as.character(AUC_pvalue))) +
      ggplot2::scale_color_manual(breaks=color_break,
                                  values=color) +
      ggplot2::labs(color='cv_fold',
                    y='sensitivity',
                    x="1-specificity",
                    title=stringr::str_c('ROC curve for ', 
                                         as.character(feature_n), 
                                         ' feature model')) +
      ggthemes::theme_few() 
  }
