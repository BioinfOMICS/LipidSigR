...
PLSDA <- function(exp_transform_table, group_info=NULL, sig_feature=NULL,
  ...
  if(plotly == TRUE){
    ...
    variable.plot <- ggplot2::ggplot() +
      ggforce::geom_circle(data=circles,
                  ggplot2::aes(x0=x0, y0=y0, r=r), show.legend = FALSE) +
      ggplot2::geom_text(data=variable.tab,
                ggplot2::aes(x=x, y=y, label=names), colour='#8B0000',
                check_overlap=FALSE, size=4, show.legend=FALSE) +
      ggplot2::theme_bw() +
