#' @title DE_species_2
#' @description Compute differentially expressed analysis of two groups (independent) based on the input Group Information to find significant lipid species. To find differentially expressed lipid species, a statistical method (t-test or Wilcoxon test) and the cut-offs for significant lipid species need to be chosen, and the p-value will then be adjusted by Benjamini-Hochberg or other methods.
...
#' @param data_transform Logical. If data_transform = TRUE,  transform exp_data by log10.
...
#' @param paired Logical. If paired = T, data are paired samples. (default: F)
#' @param test A character string indicating which method to be used for comparing means. Allowed method include \bold{"t.test"} and \bold{"wilcox.test"}. (default: "t.test")
...
#' \enumerate{
#' \item DE_species_table_all: a data frame comprises the results of differential expression analysis, including fold change, p-value, adjusted p-value.
#' \item DE_species_table_sig: a data frame comprises the significant results of differential expression analysis, including fold change, p-value, adjusted p-value.
#' \item DE_species_dotchart_sig: a lollipop chart reveals the lipid species that pass chosen cut-offs.
...
#' DE_species_2(exp_transform_non_log, data_transform = T, group_info = group_info, paired = F,
...
DE_species_2 <- function(exp_data, data_transform = T, group_info, paired = F, test = 't.test',
  ...
  if(data_transform == T & test == 't.test'){
    ...
    exp_data_tab <- exp_data_ga %>%
      ...
      dplyr::mutate(mean_ctrl = mean(unlist(ctrl), na.rm = T),
    ...
    exp_data_p <- exp_data_trans_ga %>%
      ...
      tidyr::spread(group, value) %>%
      dplyr::group_by(feature) %>%
      dplyr::mutate(p_value = tryCatch(stats::t.test(unlist(ctrl), unlist(exp), paired = paired, var.equal = T)$p.value,
                                error=function(e){NA}),
    ...
    exp_data_p <- exp_data_ga %>%
      ...
      dplyr::mutate(mean_ctrl = mean(unlist(ctrl), na.rm = T),
             ...
             FC = mean_exp/mean_ctrl,
             log2FC = log2(FC),
             p_value = tryCatch(stats::t.test(unlist(ctrl), unlist(exp), paired = paired, var.equal = T)$p.value,
                                error=function(e){NA}),
             m_log10_p_value = -log10(p_value))
    exp_data_p$p_adj = stats::p.adjust(exp_data_p$p_value, method = adjust_p_method)
    exp_data_p$m_log10_p_adj = -log10(exp_data_p$p_adj)
  ...
  if(sum(!is.na(exp_data_stat[,'p_value']))==0){
    ...
    warning('Only two samples could not be tested, significant lipids were instead defined by log2 fold change.')
  }else{
    if(sig_stat == 'p'){
      sig.diff.exp <- exp_data_stat %>%
  ...
  if(nrow(sig.diff.exp) > 0){
    ...
    if(sum(is.finite(sig.diff.exp$log2FC)) == 0){ ##all sig. lipid is infinite
      ...
      p.dotchart <- if(sum(!is.na(sig.diff.exp[,'p_value']))==0){
        p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>",
          ...
          ggplot2::scale_y_continuous(breaks = c(-5, -2, -1, 0, 1, 2, 5), labels = c('-Inf', -2, -1, 0, 1, 2, 'Inf'), limits = c(-6, 6))
      }else if(sig_stat == "p"){
        p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>","-log10(p-value) :",m_log10_p_value),color = m_log10_p_value,size=2.5)) +
          ggplot2::guides(size = "none") + ggplot2::labs(colour="-log10(p-value)") +
          ggplot2::scale_colour_gradient2(low="steelblue", mid="white", high="red", midpoint=0) +
          ggplot2::scale_y_continuous(breaks = c(-5, -2, -1, 0, 1, 2, 5), labels = c('-Inf', -2, -1, 0, 1, 2, 'Inf'), limits = c(-6, 6))
      }else{
        p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature:",feature,"<br>","log2(FC): ",
                                             round(log2FC,2),"<br>","-log10(padj) :",m_log10_p_adj),color = m_log10_p_adj,size=2.5)) +
          ggplot2::guides(size = "none") + ggplot2::labs(colour="-log10(padj)") +
          ggplot2::scale_colour_gradient2(low="steelblue", mid="white", high="red", midpoint=0) +
          ggplot2::scale_y_continuous(breaks = c(-5, -2, -1, 0, 1, 2, 5), labels = c('-Inf', -2, -1, 0, 1, 2, 'Inf'), limits = c(-6, 6))
      }
      ...
      p.dotchart <- if(sum(!is.na(sig.diff.exp[,'p_value']))==0){
        ...
        if(nrow(sig.diff.exp)==1){
          p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>",
                                                                   "-log10(padj) :",m_log10_p_adj),color = ifelse(m_log10_p_adj>0,"red","steelblue"),size=2.5)) +
            ggplot2::guides(size = "none") + ggplot2::theme(legend.position = "none")
        }else{
          p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>",
      ...
      INF <- ceiling(max(rm.inf$log2FC, na.rm = T) * 1.5)
      ...
      p.dotchart <- if(sum(!is.na(sig.diff.exp[,'p_value']))==0){
        ...
        p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>",
          ...
          ggplot2::guides(size = "none") + ggplot2::labs(colour="-log10(p-value)") +
          ggplot2::scale_colour_gradient2(low="steelblue", mid="white", high="red", midpoint=0) +
          ggplot2::scale_y_continuous(breaks = c(-INF, -2, -1, 0, 1, 2, INF), labels = c('-Inf', -2, -1, 0, 1, 2, 'Inf'), limits = c(-(INF+1), (INF+1)))
      }else{
        p.dotchart + ggplot2::geom_point(ggplot2::aes(text=paste("feature :",feature,"<br>","log2(FC) : ",round(log2FC,2),"<br>",
                                               "-log10(padj) :",m_log10_p_adj),color = m_log10_p_adj,size=2.5)) +
          ggplot2::guides(size = "none") + ggplot2::labs(colour="-log10(padj)") +
  ...
  plot_MA_Vol <-if(sum(!is.na(exp_data_stat[,'p_value']))==0){
    ...
    exp_data_stat %>%
      ...
      dplyr::mutate(A = (log2(mean_exp) + log2(mean_ctrl))/2,
             ...
             sig_log2fc.pvalue_colors = if (sig_stat == "p"){
               ifelse( M>log2(sig_FC) & -log10(p_value) > -log10(sig_pvalue), 'up-regulated',
                       ifelse(-M>log2(sig_FC) & -log10(p_value) > -log10(sig_pvalue),'down-regulated','none'))
             }else{
               ifelse( M > log2(sig_FC) & -log10(p_adj) > -log10(sig_pvalue), 'up-regulated',
                       ifelse(-M > log2(sig_FC) & -log10(p_adj) > -log10(sig_pvalue),'down-regulated','none'))
             })
  }
  plot_MA_Vol$sig_log2fc.pvalue_colors <- factor(plot_MA_Vol$sig_log2fc.pvalue_colors,levels = c("down-regulated", "none", "up-regulated"))
  DE.species.maplot <- plotly::plot_ly(data = plot_MA_Vol,
    ...
    plotly::layout(xaxis = list(title = "A = (log<sub>2</sub>(exp)+log<sub>2</sub>(ctrl))/2"),
