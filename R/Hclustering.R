...
#' Hclustering(exp_transform, DE_result_table = DE_species_table_sig, group_info = group_info,
      ...
      if(min(exp.mat.all)>=0 || max(exp.mat.all)<=0){
        if(ncol(exp.mat.all)<=50){
          heatmap.all <- iheatmapr::iheatmap(exp.mat.all,colors = heatmap_color_scale(exp.mat.all),colorbar_grid = cb_grid,scale = "rows") %>%
            ...
            iheatmapr::add_col_annotation(annotation = colGroup,side="top",show_colorbar = F) %>%
            iheatmapr::add_col_dendro(col_dend,side="top",reorder =T) %>%
            iheatmapr::add_row_annotation(annotation = rowGroup.all,side="right",show_colorbar = F) %>%
            iheatmapr::add_row_dendro(row_dend,side="right",reorder =T)
        }else{
          heatmap.all <- iheatmapr::iheatmap(exp.mat.all,colors = heatmap_color_scale(exp.mat.all),colorbar_grid = cb_grid,scale = "rows") %>%
            iheatmapr::add_col_annotation(annotation = colGroup,side="top",show_colorbar = F) %>%
            iheatmapr::add_col_dendro(col_dend,side="top",reorder =T) %>%
      ...
      if(min(exp.mat.all)>0 || max(exp.mat.all)<0){
        if(ncol(exp.mat.all)<=50){
          heatmap.all <- iheatmapr::iheatmap(exp.mat.all,colors = heatmap_color_scale(exp.mat.all),colorbar_grid = cb_grid,scale = "rows") %>%
            ...
            iheatmapr::add_col_annotation(annotation = colGroup,show_colorbar = F) %>%
            iheatmapr::add_col_dendro(col_dend,reorder =T) %>%
            iheatmapr::add_row_dendro(row_dend,side="right",reorder =T)
        }else{
          heatmap.all <- iheatmapr::iheatmap(exp.mat.all,colors = heatmap_color_scale(exp.mat.all),colorbar_grid = cb_grid,scale = "rows") %>%
            iheatmapr::add_col_annotation(annotation = colGroup,show_colorbar = F) %>%
      ...
      if(min(exp.mat.sig)>0 || max(exp.mat.sig)<0){
        if(ncol(exp.mat.sig)<=50){
          heatmap.sig <- iheatmapr::iheatmap(exp.mat.sig,colors = heatmap_color_scale(exp.mat.sig),colorbar_grid = cb_grid,scale = "rows") %>%
            ...
            iheatmapr::add_col_annotation(annotation = colGroup,show_colorbar = F) %>%
            iheatmapr::add_col_dendro(col_dend,reorder =T) %>%
            iheatmapr::add_row_annotation(annotation = rowGroup.sig,show_colorbar = F) %>%
            iheatmapr::add_row_dendro(row_dend,side="right",reorder =T)
        }else{
          heatmap.sig <- iheatmapr::iheatmap(exp.mat.sig,colors = heatmap_color_scale(exp.mat.sig),colorbar_grid = cb_grid,scale = "rows") %>%
            iheatmapr::add_col_annotation(annotation = colGroup,show_colorbar = F) %>%
