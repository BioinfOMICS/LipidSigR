...
.two_way_anova <- function(abundance, group_info, char=NULL){
    ...
    colnames(abundance)[seq_len(2)] <- c('characteristic', 'feature')
    test.all <- NULL
    if(is.null(char)){
...
.char_index <- function(abundance, char){
    ...
    combined.abund <- purrr::map2(
        abund[1], abund[-1], ~sum(.x*.y, na.rm=TRUE)/sum(.y, na.rm=TRUE)) %>%
...
.deChar_bar_multiGroup_plotly <- function(plot_tab, bar){
    ...
    for(i in seq_len(length(bar.plotly$x$data))){
        for(j in seq_len(length(bar.plotly$x$data[[i]]$text))){
            if(i %in% seq_len(length(unique(plot_tab$group))*2)){
...
.deChar_line_multiGroup_plotly <- function(plot_tab, line.plot){
    ...
    for(i in seq_len(length(line.plotly$x$data))){
        if(i == (length(unique(plot_tab$group))+1)){
            line.plotly$x$data[[i]]$text <- ""
        }else{
            for(j in seq_len(length(line.plotly$x$data[[i]]$text))){
                if(i %in% seq_len(length(unique(plot_tab$group)))){
...
.box_stat_multiGroup <- function(sub_abund, group_info, ref_group, test){
    ...
    if(is.null(test.res)){
        ...
        com.all <- NULL
        for(i in seq_len(length(comparison))){
