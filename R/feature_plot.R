...
#' @param data1 feature after cross-validation. An output list of
#' \code{\link{ML_final}} (list[[6]], cv_feature_save).
#' @param data2 variable importance after cross-validation. An output
#' data frame of \code{\link{ML_final}} (list[[7]],cv_varimp_result).
#' @param feature_n A numeric value specifying the number of elements
#' to be shown.
#' @param nfold A numeric value. An optional parameter for setting the number
...
feature_plot <- function(data1, data2, feature_n, nfold=10,
  ...
  feature_freq_data <- data.frame(feature=names(cv_sele_feature),
                                  sele_freq=
                                    as.numeric(cv_sele_feature)/nfold) %>%
  ...
  #new
  feature_freq_data <- feature_freq_data %>%
    dplyr::arrange(dplyr::desc(sele_freq))
  #
  if(plotly == TRUE){
    feature_freq_plot <- feature_freq_data[seq_len(plot_feature_num), ] %>%
      plotly::plot_ly(x=~(sele_freq*100),
                      y=~stats::reorder(feature, sele_freq),
                      type="bar", orientation="h",
                      marker=list(color=~-sele_freq, colorscale="Blues"),
                      text=~paste("Feature :", feature,
                                  "<br>Selected frequency :",
                                  sele_freq*100, "%"), hoverinfo="text") %>%
      plotly::layout(xaxis=list(title="Selected frequency (%)",
                                showgrid=TRUE, nticks=20,
                                showline=TRUE, mirror='all'),
                     yaxis=list(title=""))
  }else{
    feature_freq_plot <- feature_freq_data[seq_len(plot_feature_num), ] %>%
      ggplot2::ggplot(ggplot2::aes(x=(sele_freq*100),
                                   y=stats::reorder(feature, sele_freq),
                                   fill=-sele_freq)) +
      ggplot2::geom_col() +
      ggplot2::labs(y='',
  ...
  if(plotly == TRUE){
    feature_imp_plot <- feature_imp_data[seq_len(plot_feature_num), ] %>%
      plotly::plot_ly(x=~importance,y=~stats::reorder(feature, importance),
                      type="bar", orientation="h",
                      marker=list(color=~-importance, colorscale="Blues"),
                      text=~paste("Feature :", feature,
                                  "<br>Average importance :",
                                  round(importance, 2)), hoverinfo="text") %>%
      plotly::layout(xaxis=list(title='Average importance',
                                showgrid=TRUE, nticks=20,
                                showline=TRUE, mirror='all'),
                     yaxis=list(title=""))
  }else{
