...
#' @description This function provides two plots depending on the
...
exp_compo_by_lipidinfo <- function(exp_data, lipid_char_table,
                                   char_var, plotly=TRUE){
  ...
  if(plotly==TRUE){
    p.barplot.p <-merge(exp_data %>%
      ...
      plotly::plot_ly(x=~lipid.category.value,
                      ...
                      type="bar",
                      hoverinfo="text",
                      text=~paste(Hmisc::capitalize(char_var)," :",
                                  lipid.category.value,
    ...
    p.barplot.p <- merge(exp_data %>% tidyr::gather(sample_name, value,
                                                    -feature),
                        lipid_char_table,
                        by="feature") %>%
      dplyr::filter(lipid.category == char_var &
      ...
      dplyr::mutate(value=value*weight)%>%
      plotly::ungroup() %>%
      ggplot2::ggplot(ggplot2::aes(x=lipid.category.value,
                                   y=value,
                                   fill=sample_name)) +
      ggplot2::geom_col(position = "dodge") +
      ggplot2::labs(fill="Sample name",
                    y='Lipid Expression',
                    x=Hmisc::capitalize(char_var),
                    title=paste0('Lipid ', Hmisc::capitalize(char_var))) +
      ggthemes::theme_hc()
  }
  ...
  if(plotly==TRUE){
    ...
    p.compos <- merge(exp_data %>% tidyr::gather(sample_name,value,-feature),
      ...
      ggplot2::ggplot(ggplot2::aes(x=value,
                                   y=sample_name,
                                   fill=lipid.category.value)) +
      ggplot2::geom_col() +
      ggplot2::labs(fill=Hmisc::capitalize(char_var),
                    y='',
                    x="%",
                    title=paste0('Lipid ', Hmisc::capitalize(char_var),
                                 ' Composition')) +
