...
exp_compo_by_lipidinfo <- function(exp_data, lipid_char_table, char_var){
  ...
  if(length(FA_col)>0){
    ...
    for(i in seq_len(length(FA_col))){
      col <- FA_col[i]
      comma_count <- max(stringr::str_count(lipid_char_table[,col], ','),na.rm = TRUE)
      if(comma_count>0){
        lipid_char_table <- tidyr::separate(lipid_char_table,col,c(col,paste0(col,"_",seq_len(comma_count))))
      }
      if(comma_count>max_comma){max_comma <- comma_count}
    }
