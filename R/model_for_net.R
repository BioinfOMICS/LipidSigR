...
#' @param varimp_method A character string indicating the method to input feature importance. Allowed methods are \bold{Algorithm-based} and \bold{SHAP}.
#' @param best_model The output list of \code{\link{ML_final}}.
#' @param best_model_feature The ouput list of \code{\link{ML_final}}.
#' @param feature_num A numeric value specifying the number of features to be computed.
...
#' model_for_net(ML_data[[2]], ML_method='Random_forest', varimp_method='Algorithm-based',
#'               ML_output[[8]], ML_output[[9]], feature_num=10, nsim=5)
#' }
model_for_net <- function(data, ML_method, varimp_method, best_model,
  ...
  if(!class(feature_num)%in%c("numeric","integer")){
    ...
    if(!feature_num %in% names(best_model)){
      stop(paste0('feature_num must be one of ',paste(names(best_model),collapse=", ")))
    }
  }
  ...
  if(varimp_method=='Algorithm-based'){
    ...
    if(ML_method=='Random_forest'){
      ...
      varimp <- stats::coef(best_model[[num]],s = 'lambda.min') %>% as.matrix() %>% as.data.frame()%>% .[-1, ,drop=F]
    ...
    varImp_dir <- varImp_result %>% dplyr::mutate(dir=shapley_value*raw_value) %>%
      dplyr::group_by(variable) %>%
      dplyr::summarise(dir=sum(dir))
    varImp_result <- varImp_result[c(2,6)] %>% unique() %>%
      dplyr::left_join(varImp_dir,by='variable') %>%
      dplyr::mutate(mean_shapley_value=ifelse(dir>0, mean_shapley_value, -mean_shapley_value))
