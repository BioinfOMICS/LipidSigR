...
#' @description This function provides data preprocessing and output processed
...
#' @param group_info A data frame of one clinical term
#' (or one set of clinical terms) from demographic data.
#' The first column is the sample name and the second column is the group
#' number assigned to each sample. The group number can only be 0 or 1.
#' NAs are not allowed.
...
#' certain percentage should be removed.
...
#' @param pct_transform Logical. Transform lipid value into a percentage.
#' (default:TRUE)
#' @param data_transform Logical. Data transformation(log).
#' (default:TRUE)
#' @param trans_type A character string of data transformation type.
#' (default:'log')
#' @param centering A logical value indicating whether the variables
#' should be shifted to be zero centered. Alternately, a vector of length
...
#' \enumerate{
#' \item [[1]]: the processed data can be used as input of machine
#' learning analysis. The value of this data frame is adding the
#' lipid (feature)'s original expression to the expression of user-selected
#' lipid characteristics.
#' \item [[2]]: The transpose data frame of \bold{[[1]]}, which is also
#' processed data and can be used as the input of machine learning analysis.
#' The value of this data frame is adding the lipid (feature)'s original
#' expression to the expression of user-selected lipid characteristics.
#' }
#' @export
#' @examples
#' data("ML_exp_data")
...
ML_data_process <- function(exp_data, group_info, lipid_char_table,
                            char_var=NULL, exclude_var_missing=TRUE,
  ...
  if(ncol(group_info) == 2){
    if(is(group_info[, 1], 'character') &
       sum(group_info$group %in% c(0, 1)) == nrow(group_info)){
      if(sum(table(group_info[, 2]) >= 30) != 2){
        stop("group_info each group must have more than 30 samples.")
      }
    }else{
      stop("group_info columns 'sample_name' must be characters; the column
           'group' must be numeric (only be 0 or 1).")
    }
  }else{
    stop("group_info column must contain 'sample_name' (1st column),
         'group' (2nd column)")
  }
  if(sum(group_info[, 1] %in% colnames(exp_data)) != nrow(group_info) |
     sum(group_info[, 1] %in% colnames(exp_data)) != ncol(exp_data[, -1])){
    stop("group_info column 'sample_name' must same as the name of
         samples of exp_data")
  }
  ...
  if((!is.null(lipid_char_table)) && (!is.null(char_var))){
    ...
    for(a in seq_len(char_num)){
      char_data[[a]] <- Species2Char(exp_data, lipid_char_table,
                                     char_var[a]) %>%
        dplyr::mutate(type=char_var[a])
      char_data[[a]][[1]] <- stringr::str_c(char_var[a], '_',
                                            char_data[[a]][[1]])
  ...
  for(var_num in seq_len(length(combo))){
    trans_data <- ML_data[[1]] %>%
      dplyr::filter(type==combo[var_num]) %>%
      dplyr::select(-type)
    char_data[[var_num]] <- data_process(trans_data,
                                         exclude_var_missing=
                                           exclude_var_missing,
  ...
  ML_data[[2]] <- data[-1] %>% t() %>%
    as.data.frame() %>% dplyr::mutate(group=ML_data[[2]]$group) %>%
