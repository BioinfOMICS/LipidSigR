...
#' @param pct_transform Logical. Transform lipid value into a percentage. (default:T)
#' @param data_transform Logical. Data transformation(log). (default:T)
#' @param trans_type A character string of data transformation type. (default:'log')
#' @param centering A logical value indicating whether the variables should be shifted to be zero centered. Alternately, a vector of length equal the number of columns of x can be supplied. The value is passed to scale. (default: F)
...
#' ML_data_process(exp_data, group_info = condition_table, lipid_char_table, char_var[1],
#'                 exclude_var_missing=T, missing_pct_limit=50, replace_zero=T,
#'                 zero2what='min', xmin=0.5,replace_NA=T, NA2what='min', ymin=0.5,
#'                 pct_transform=T, data_transform=T, trans_type='log',
...
ML_data_process <- function(exp_data, group_info, lipid_char_table, char_var=NULL,
  ...
  if(ncol(group_info)==2){
    if(class(group_info[,1])=="character" & sum(group_info$group%in%c(0,1))==nrow(group_info)){
