...
pre-processing step. In t-SNE, \Robject{perplexity} and \Robject{max\_iter}
are adjustable for users. The \Robject{perplexity} may be considered as a knob
that sets the number of effective nearest neighbours, while \Robject{max\_iter}
...
that is to group neighbouring data points which certainly delivers a very
...
applying pre-processing step (such as PCA). Besides, UMAP applies Graph
...
\Robject{hclustfun}. Two heatmaps will be shown by lipid species and by samples.
...
@
\begin{figure}
  \includegraphics[width=0.65\textwidth]{images/lipids_hm.png}
  ...
  \label{fig:corr_heamtap_lipidshm}
\end{figure}
...
DE_species_result <- DE_species_2(exp_transform_non_log,
                                  ...
                                  sig_pvalue=0.05, sig_FC=2, plotly=FALSE)
@
...
@
  ...
  \item \textbf{t-SNE (t-Distributed Stochastic Neighbour Embedding)}\\
    t-Distributed Stochastic Neighbour Embedding (t-SNE) is an unsupervised
    non-linear dimensionality reduction technique that tries to retain the local
    structure(cluster) of data when visualising the high-dimensional datasets.
...
visualized on heatmap by hierarchical clustering.
...
@
\begin{figure}
  \includegraphics[width=0.65\textwidth]{images/DEspec_sig.lipid.png}
  \caption{\textbf{Heatmap of significant lipid species}}
  \label{fig:DEspec_sig.lipid}
\end{figure}
...
on the heatmap using hierarchical clustering.
...
DEchar_Hcluster <- Hclustering(exp_transform_class, DE_char_table_sig,
                               ...
                               hclustfun='complete', plotly=FALSE)
@
...
@
\begin{figure}
  \includegraphics[width=0.65\textwidth]{images/DEchar_sig.lipid.png}
  \caption{\textbf{Heatmap of significant lipid species}}
  \label{fig:DEchar_sig.lipid}
\end{figure}
