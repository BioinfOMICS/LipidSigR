...
%\newcommand\knitr{\Rpackage{Sweave2knitr}}
...
\textbf{"LipidSigR"} is an R package developed based on LipidSig web-based tool (\url{http://www.chenglab.cmu.edu.tw/lipidsig/}) \cite{lipidsig}. This package integrates a comprehensive analysis for streamlined data mining of lipidomic datasets. We provide 4 main analysis workflows, which is \textbf{"Profiling"}, \textbf{"Differential expression"}, \textbf{"Machine learning"}, and \textbf{"Correlation"}. Each section provides unique aspects to analyze the lipidome profiling data based on different characteristics including lipid class, chain length, unsaturation, hydroxyl group, and fatty acid composition.\\

The usage of the 4 sections is introduced briefly as below.

\begin{enumerate}
     \item \textbf{Profiling}: The profiling section provides an overview of comprehensive analyses for you to efficiently examine data quality, the clustering of samples, the correlation between lipid species, and the composition of lipid characteristics.
      \item \textbf{Differential expression}: The differential expression section integrates many useful lipid-focused analyses for identifying significant lipid species or lipid characteristics.
      \item \textbf{Machine learning}: The machine learning section provides a broad variety of feature selection methods and classifiers to build binary classification models. Furthermore, the subsequent analyses can help users to evaluate the learning algorithm's performance and to explore important lipid-related variables.
      \item \textbf{Correlation}: The correlation section illustrates and compares the relationships between different clinical phenotypes and lipid features.
\end{enumerate}
...
This section briefly describes the procedures of running the \textbf{LipidSigR} package on your system. We assume that you have already installed the R program (see the R project at \url{http://www.r-project.org})  and are familiar with it. You need to have R 4.0.0 or a later version installed for running LipidSigR.

The \textbf{LipidSigR} package is available at the Bioconductor repository (\url{http://www.bioconductor.org}). To install our package, first, you need to install the core Bioconductor packages. If you have already installed the Bioconductor packages on your system, you can skip the following step below.
...
First, we have to read the input data needed for the profiling section. We have to prepare lipid expression data and lipid characteristics (optional) as the input data of \Robject{exp\_data} and \Robject{lipid\_char\_table}. Please note that \Robject{lipid\_char\_table} only be used in Section \ref{subsec:pro_char}.
...
After importing the input data, sometimes, we may need to conduct data processing before analysis. Here, we provide the \Rfunction{data\_process} function for data processing, including removing features with missing values, missing values imputation, percentage transformation, log10 transformation, etc.
...
@
After conduct the above code, you will get a list \Robject{profiling\_result} with three types of distribution plots.
<<numExpLipid>>=
# view result: histograms (number of expressed lipids)
profiling_result$i.expr.lip
@
\begin{figure}
   \includegraphics[width=0.7\textwidth]{images/exp_profilling_1.jpeg}
   \caption{\textbf{Histogram of number of expressed lipids}\\ The histogram overviews the total number of lipid species over samples. From the plot, we can discover the number of lipid species present in each sample.}
   \label{fig:num_exp_lipid}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/exp_profilling_2.jpeg}
  \caption{\textbf{Histogram of lipid amount}\\ The histogram describes the variability of the total lipid amount between samples.}
  \label{fig:lipid_amount}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/exp_profilling_3.jpeg}
  \caption{\textbf{Density plot of expression distribution}\\ The density plot uncovers the distribution of lipid expression in each sample (line). All expression was log10 transformed. From this plot, we can have a deeper view of the distribution between samples.}
  \label{fig:desity_exp_distri}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{Clustering methods}: K-means, partitioning around medoids (PAM), Hierarchical clustering, and DBSCAN
\end{itemize}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/pca_biplot.jpeg}
  \caption{\textbf{PCA plot}}
  \label{fig:PCA_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/pca_screeplot.jpeg}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/pca_variable.jpeg}
  \caption{\textbf{Correlation circle plot}\\ The correlation circle plot showing the correlation between a feature (lipid species) and a principal component (PC) used as the coordinates of the variable on the PC \cite{abdi2010principal}. The positively correlated variables are in the same quadrants while negatively correlated variables are on the opposite sides of the plot origin. The closer a variable to the edge of the circle, the better it represents on the factor map.}
...
@
\begin{figure}
  \includegraphics[width=0.6\textwidth]{images/pca_feature_contrib.jpeg}
  \caption{\textbf{Bar plot of contribution of top 10 features}}
  \label{fig:PCA_contribution}
\end{figure}

\subsubsection{t-SNE}
\label{t-SNE}
t-SNE (t-Distributed Stochastic Neighbour Embedding) is an unsupervised non-linear dimensionality reduction technique that tries to retain the local structure(cluster) of data when visualising the high-dimensional datasets. Package \CRANpkg{Rtsne} is used for calculation, and PCA is applied as a pre-processing step. In t-SNE, \Robject{perplexity} and \Robject{max\_iter} are adjustable for users. The \Robject{perplexity} may be considered as a knob that sets the number of effective nearest neighbours, while \Robject{max\_iter} is the maximum number of iterations to perform. The typical perplexity range between 5 and 50, but if the t-SNE plot shows a ‘ball’ with uniformly distributed points, you may need to lower your perplexity \cite{van2008visualizing}.
...
@
\begin{figure}
  \includegraphics[width=0.6\textwidth]{images/UMAP_plot.jpeg}
  \caption{\textbf{UMAP plot}}
  \label{fig:UMAP_plot}
\end{figure}

\subsection{Correlation heatmap}
The correlation heatmap illustrates the correlation between samples or lipid species and also depicts the patterns in each group. The correlation is calculated by the method defined by parameter \Robject{corr\_method}, and the correlation coefficient is then clustered depending on method defined by parameter \Robject{distfun} and the distance defined by parameter \Robject{hclustfun}. Two heatmaps will be shown by lipid species and by samples. Please note that if the number of lipids or samples is over 50, the names of lipids/samples will not be shown on the heatmap.
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/corr_heatmap_1.jpeg}
  \caption{\textbf{Heatmap of sample to sample correlations}\\ The heatmap reveals sample to sample correlations. Correlations between lipid species are colored from strong positive correlations (red) to no correlation (white).}
  \label{fig:corr_heatmap_SS}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/corr_heatmap_2.jpeg}
  \caption{\textbf{Heatmap of lipid to lipid correlations}\\ The heatmap illustrates the lipid to lipid correlations. Correlations between lipid species are colored from strong positive correlation (red) to no correlation (white), to negative correlation (blue).}
  \label{fig:corr_heatmap_LL}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/exp_compo_by_lipidinfo_01.jpeg}
  \caption{\textbf{Bar plot classified by selected characteristic}\\  The bar plot depicts the expression level of each sample within each group (e.g., PE, PC) of selected characteristics (e.g., class).}
  \label{fig:exp_compo_by_lipidinfo_1}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/exp_compo_by_lipidinfo_02.jpeg}
  \caption{\textbf{Lipid class composition}\\  The stacked horizontal bar chart illustrates the percentage of characteristics in each sample. The variability of percentage between samples can also be obtained from this plot.}
  \label{fig:exp_compo_by_lipidinfo_2}
\end{figure}
...
\begin{itemize}
  \item \textbf{Lipid species analysis}: The lipid species analysis explores the significant lipid species based on differentially expressed analysis. Data are analyzed based on each lipid species. Further analysis and visualization methods, include
     \begin{enumerate}
...
\subsection{Input data}
First, we have to read the input data needed for the differential expression section. We have to prepare lipid expression data (\Robject{exp\_data}), lipid characteristics table (\Robject{lipid\_char\_table}), and a table of one clinical term (or one set of clinical terms) from demographic data (\Robject{group\_info}) as input data.
...
The above \Robject{DE\_species\_table\_sig} data frame will be used in the following analyses in Section \ref{DEdimensionReduce}, Section \ref{hierarchicalCluster}, Section \ref{speciesChar}, and Section \ref{enrichment}.
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_species_dotchart_sig.jpeg}
  \caption{\textbf{Lollipop chart of lipid species analysis}\\  The lollipop chart reveals the lipid species that pass chosen cut-offs. The x-axis shows log2 fold change while the y-axis is a list of lipids species. The color of the point is determined by \Robject{-log10(adj\_value/p-value)}.}
  \label{fig:DE_species_2_dotchart}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_species_maplot.jpeg}
  \caption{\textbf{MA plot}\\  The MA plot indicates three groups of lipid species, up-regulated(red), down-regulated(blue), and non-significant(grey).}
  \label{fig:DE_species_maplot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_species_volcano.jpeg}
  \caption{\textbf{Volcano plot}\\  The volcano plot illustrates a similar concept to the MA plot. These points visually identify the most biologically significant lipid species (red for up-regulated, blue for down-regulated, and grey for non-significant).}
  \label{fig:DE_species_volcano}
\end{figure}
...
\begin{itemize}
  \item Note: The input data of this section should be by filtered by function \Rfunction{DE\_species\_2}.
\end{itemize}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_PCA_plot}
  \caption{\textbf{PCA Results - PCA plot}}
  \label{fig:DEs_PCA_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_PCA_scree_plot.jpeg}
  \caption{\textbf{PCA Results - scree plot}}
  \label{fig:DEs_PCA_scree_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_PCA_correlation_circle.jpeg}
  \caption{\textbf{PCA Results - bar plot}}
  \label{fig:DEs_PCA_correlation_circle}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_PCA_contribution.jpeg}
  \caption{\textbf{PCA Results - correlation circle plot}}
  \label{fig:DEs_PCA_contribution}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_tsne_plot.jpeg}
  \caption{\textbf{t-SNE plot}}
  \label{fig:DEs_tsne_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEs_UMAP_plot.jpeg}
  \caption{\textbf{UMAP plot}}
  \label{fig:DEs_UMAP_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/PLSDA_sample.plot.jpeg}
  \caption{\textbf{Sample plot}}
  \label{fig:PLS-DA_sample}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/PLSDA_variable.plot.jpeg}
  \caption{\textbf{Loading plot}\\ In the PLS-DA loading plot, the distance to the center of the variables indicates the contribution of the variable. The value of the x-axis reveals the contribution of the variable to PLS-DA-1, whereas the value of the y-axis discloses the contribution of the variable to PLS-DA-2.}
  \label{fig:PLS-DA_loding}
\end{figure}
...
The top of the heatmap is grouped by sample group (top annotation) while the side of the heatmap (row annotation) can be chosen from \Robject{lipid\_char\_table}, such as class, structural category, functional category, total length, total double bond (totaldb), hydroxyl group number (totaloh), fatty acid length (FA\_length), the double bond of fatty acid(FA\_db), hydroxyl group number of fatty acid(FA\_oh).
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/Hclustering_all.lipid.jpeg}
  \caption{\textbf{Heatmap of all lipid species}}
  \label{fig:Hclustering_all.lipid}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/Hclustering_sig.lipid.jpeg}
  \caption{\textbf{Heatmap of significant lipid species}\\ Through heatmap, we can discover the difference between the two groups by observing the distribution of lipid species.}
  \label{fig:Hclustering_sig.lipid}
\end{figure}
...
\label{speciesChar}
The characteristics analysis visualizes the difference between control and experimental groups of significant lipid species categorized based on different lipid characteristics from \Robject{lipid\_char\_table}, such as class, structural category, functional category, total length, total double bond (totaldb), hydroxyl group number (totaloh), fatty acid length (FA\_length), the double bond of fatty acid (FA\_db), hydroxyl group number of fatty acid (FA\_oh).
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/Sig_lipid_barplot.jpeg}
  \caption{\textbf{The bar chart of significant groups}\\  The bar chart shows the significant groups (values) with mean fold change over 2 in the selected characteristics by colors (red for significant and black for insignificant).}
  \label{fig:Sig_lipid_feature_barplot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/Sig_lipid_lollipop.jpeg}
  \caption{\textbf{The lollipop chart of all significant groups}\\  The lollipop chart compares multiple values simultaneously and it aligns the log2(fold change) of all significant groups (values) within the selected characteristics.}
  \label{fig:Sig_lipid_feature_lollipop}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/Sig_lipid_word.jpeg}
  \caption{\textbf{Word cloud with the count of each group}\\ The word cloud shows the count of each group(value) of the selected characteristics.}
  \label{fig:Sig_lipid_feature_word}
\end{figure}
...
@
\begin{itemize}
  \item Note: A lipid species may have more than one fatty acid attached; thus, if the selected lipid characteristics are FA-related terms, we decompose lipid species into FA and do the enrichment instead of counting species.
\end{itemize}
...
Here, we provide data on lipid-related pathways and genes. For conducting pathway analysis, we will need the following two data, \Robject{DE.lipid.gene.path} and \Robject{DE.pathway.gene.list}.

\begin{itemize}
  \item \textbf{DE.lipid.gene.path}: A data frame of pathway information, which can be retrieved and reorganized from KEGG or other databases. It includes the related pathways of lipids, ID, pathway ID, pathway name, gene ID, and gene name.
  \item \textbf{DE.pathway.gene.list}: A list that comprises the genes included in each pathway for compiling the significant network of the selected lipid class.
\end{itemize}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{hsa00565.pathview.png}
  \caption{\textbf{An example of KEGG pathway}}
  \label{fig:hsa00565.pathview}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_char_barplot.jpeg}
  \caption{\textbf{The results of 'Characteristics' analysis in the first section - bar plot}}
  \label{fig:DE_char_2_barplot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_char_barplot_sqrt.jpeg}
  \caption{\textbf{The results of 'Characteristics' analysis in the first section - sqrt-scaled bar plot}}
  \label{fig:DE_char_barplot_sqrt}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_char_trendplot.jpeg}
  \caption{\textbf{The results of 'Characteristics' analysis in the first section - line plot}}
  \label{fig:DE_char_2_trendplot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_char_trendplot_sqrt.jpeg}
  \caption{\textbf{The results of 'Characteristics' analysis in the first section - sqrt-scaled line plot}}
  \label{fig:DE_char_trendplot_sqrt}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_char_boxplot.jpeg}
  \caption{\textbf{The results of 'Characteristics' analysis in the first section - box plot}}
  \label{fig:DE_char_2_boxplot}
\end{figure}

\newpage

In the \textbf{'Subgroup of characteristics'}, besides the selected characteristic in first section defined by parameter \Robject{char\_var}, we can further choose another characteristic by parameter \Robject{split\_var} (e.g. class). And then, analyzed results from the previous section are categorized by one of the subgroups (e.g. PC) of the selected characteristic.
...
@
\begin{itemize}
  \item \textit{Note: The star above the bar shows the significant difference of the specific subgroup of the selected characteristic between control and experimental groups.}
\end{itemize}
<<>>=
# view result: bar plot of split_class
sub_char_result[[1]]
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_sub_char_plot_2_1.jpeg}
  \caption{\textbf{The results of 'Subgroup of characteristics' analysis in the second section - bar plot}}
  \label{fig:DE_sub_char_plot_2_bar}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_sub_char_plot_2_4.jpeg}
  \caption{\textbf{The results of 'Subgroup of characteristics' analysis in the second section - sqrt-scaled bar plot}}
  \label{fig:DE_sub_char_plot_2_bar_sqrt}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_sub_char_plot_2_2.jpeg}
  \caption{\textbf{The results of 'Subgroup of characteristics' analysis in the second section - line plot}}
  \label{fig:DE_sub_char_plot_2_line}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_sub_char_plot_2_5.jpeg}
  \caption{\textbf{The results of 'Subgroup of characteristics' analysis in the second section - sqrt-scaled line plot}}
  \label{fig:DE_sub_char_plot_2_line_sqrt}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DE_sub_char_plot_2_3.jpeg}
  \caption{\textbf{The results of 'Subgroup of characteristics' analysis in the second section - box plot}}
  \label{fig:DE_sub_char_plot_2_box}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PCA_plot.jpeg}
  \caption{\textbf{Results of PCA - PCA plot}}
  \label{fig:DEc_PCA_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PCA_scree_plot.jpeg}
  \caption{\textbf{Results of PCA - scree plot}}
  \label{fig:DEc_PCA_scree_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PCA_correlation_circle.jpeg}
  \caption{\textbf{Results of PCA - bar plot}}
  \label{fig:DEc_PCA_correlation_circle}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PCA_contribution.jpeg}
  \caption{\textbf{Results of PCA - correlation circle plot}}
  \label{fig:DEc_PCA_contribution}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_tsne_plot.jpeg}
  \caption{\textbf{t-SNE plot}}
  \label{fig:DEc_tsne_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_UMAP_plot.jpeg}
  \caption{\textbf{UMAP plot}}
  \label{fig:DEc_UMAP_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PLSDA_sample.plot.jpeg}
  \caption{\textbf{PLS-DA sample plot}}
  \label{fig:DEc_PLSDA_sample.plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_PLSDA_variable.plot.jpeg}
  \caption{\textbf{PLS-DA loading plot}\\ In the PLS-DA loading plot, the distance to the center of the variables indicates the contribution of the variable. The value of the x-axis reveals the contribution of the variable to PLS-DA-1, whereas the value of the y-axis discloses the contribution of the variable to PLS-DA-2.}
  \label{fig:DEc_PLSDA_variable.plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_Hclustering_all.lipid.jpeg}
  \caption{\textbf{Heatmap of all lipid species}}
  \label{fig:DEc_Hclustering_all.lipid}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/DEc_Hclustering_sig.lipid.jpeg}
  \caption{\textbf{Heatmap of all lipid species}}
  \label{fig:DEc_Hclustering_sig.lipid}
\end{figure}
...
\subsection{Input data}
First, we have to read the input data needed for the machine learning section. We have to prepare lipid expression data (\Robject{exp\_data}), lipid characteristics table (\Robject{lipid\_char\_table}), and a condition table of sample names and clinical conditions (\Robject{condition\_table}) as input data.
...
@
After importing the input data, sometimes, we may need to conduct data processing before analysis. Here, we provide the \Rfunction{ML\_data\_process} function for data processing, including removing features with missing values, missing values imputation, percentage transformation, log10 transformation, etc.
<<ML_data_process>>=
# lipid expression data
head(exp_data[, 1:5], 5)
# data processing of exp_data
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/ROC_mean_AUC_plot.jpeg}
  \caption{\textbf{ROC curve plot}\\  The plot shows the average ROC curve for different feature numbers with their mean AUC and 95\% confidence interval.}
  \label{fig:ROC_mean_AUC_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/ROC_plot.jpeg}
  \caption{\textbf{average ROC curve plot of 10 features}\\  The plot displays average ROC curves of user-defined features. Each CV is in grey, and the red line is the average of those cross-validations (CVs) for the ROC curves.}
  \label{fig:ROC_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/PR_mean_AUC_plot.jpeg}
  \caption{\textbf{PR curve plot}\\  The plot shows the average PR curve for different feature numbers with their mean AUC and 95\% confidence interval.}
  \label{fig:PR_mean_AUC_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/PR_plot.jpeg}
  \caption{\textbf{average PR curve plot of 10 features}\\ The plot displays the average PR curves of user-defined features. Each CV is in grey, and the red line is the average of those cross-validations (CVs) for the PR curves.}
  \label{fig:PR_plot}
\end{figure}
...
Here, all the provided evaluation indicators are listed below. We can define the evaluation method by the parameter \Robject{method}.
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/evalution_plot.jpeg}
  \caption{\textbf{Model performance (Accuracy)}\\  The evaluation plot shows the model performance of accuracy. The highest value is marked in red.}
  \label{fig:evalution_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/cm_plot.jpeg}
  \caption{\textbf{Confusion matrix}\\ In the confusion matrix, the y-axis indicates the predicted class, the x-axis is the actual class. Therefore, the upper left is a true positive; the upper right is a false positive; the lower left is a false negative; the lower right is a true negative. The numbers are the count and the number in the bracket is the percentage.}
  \label{fig:probability_plot_cm}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/feature_freq_plot.jpeg}
  \caption{\textbf{Feature importance (Algorithm-based)-selected frequency}}
  \label{fig:feature_freq_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/feature_imp_plot.jpeg}
  \caption{\textbf{Feature importance (Algorithm-based)-feature importance}}
  \label{fig:feature_imp_plot}
\end{figure}

\newpage

\subsubsection{SHAP analysis}
Shapley Additive exPlanations (SHAP) approach on the basis of Shapley values in game theory has recently been introduced to explain individual predictions of any machine learning model. More detailed information can be found in the paper 'A Unified Approach to Interpreting Model Predictions' (2017) \cite{lundberg2017unified}.

The analysis is based on the result of ROC-AUC and PR-AUC. We can decide the feature number by parameter \Robject{feature\_n}. According to the defined feature number, the corresponding best model in all CVs will be used to compute approximate Shapley values of each feature for all samples with \CRANpkg{fastshap} package in R.
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/mean_shapley_plot.jpeg}
  \caption{\textbf{SHAP feature importance plot}\\  The top 10 features are ranked and demonstrated according to the average absolute value of shapely values from all samples.}
  \label{fig:mean_shapley_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/SHAP_sample_plot.jpeg}
  \caption{\textbf{SHAP feature importance of 10 samples}}
  \label{fig:SHAP_sample_plot}
\end{figure}

Lastly, we build the SHAP force plot and dependence plot with different parameter sets.

The SHAP force plot stacks these Shapley values and shows how the selected features affect the final output for each sample. We can decide the number of top features to be shown by parameter \Robject{topN\_feature}, and the number of groups the samples to be clustered into by parameter \Robject{group\_num}.
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/SHAP_f_plot.jpeg}
  \caption{\textbf{SHAP force plot}\\  The colors of the bars are filled according to the features.}
  \label{fig:f_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.7\textwidth]{images/shap_dependence_plot.jpeg}
  \caption{\textbf{SHAP dependence plot}}
  \label{fig:shap_dependence_plot}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.8\textwidth]{images/corr_network.jpeg}
  \caption{\textbf{The network of feature importance}}
  \label{fig:visNetwork_plot}
\end{figure}
...
\label{corr}
In the final part, we are going conduct a comprehensive correlation analysis to interrogate the clinical features that connect to lipids species and other mechanistically relevant lipid characteristics. Correlation analysis between lipids and clinical features is broadly used in many fields of study, such as Bowler RP et al. discovering that sphingomyelins are strongly associated with emphysema and glycosphingolipids are associated with COPD exacerbations \cite{bowler2015plasma}.
...
\begin{itemize}
    \item Distance measurement: Pearson, Spearman, Kendall, Euclidean, Maximum, Manhattan, Canberra, Binary, and Minkowski.
    \item Clustering method: median, average, single, complete, Ward.D, Ward.D2, WPGMA, and UPGMC
\end{itemize}

\subsection{Input data}
First, we have to read the input data needed for the correlation analysis section. We have to prepare lipid expression data (\Robject{exp\_data}), lipid characteristics table (\Robject{lipid\_char\_table}), a condition table of sample names and clinical conditions (\Robject{condition\_table}), and an adjusted table with additional variables for adjusting confounding effects (\Robject{adjusted\_table}) as input data.
...
@
\begin{figure}
  \includegraphics[width=0.8\textwidth]{images/Clin_Cor_table_plot.jpeg}
  \caption{\textbf{Correlation coefficient for lipid species analysis}\\  Only the variables that pass the defined cut-offs for p-value and the correlation coefficient are shown on the heatmap. The rows of the heatmap are clinical features and the columns are the lipid species.}
  \label{fig:Clin_Cor_heatmap}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.8\textwidth]{images/Clin_LR_table_plot.jpeg}
  \caption{\textbf{The heatmap of linear regression for lipid species analysis}\\  Only the variables that pass the user-defined cut-offs for p-value and the correlation coefficient are shown on the heatmap. The rows of the heatmap are clinical features and the columns are the lipid species.}
  \label{fig:Clin_LR_heatmap}
\end{figure}
...
\subsubsection{Correlation}
his section provides the correlation of lipid characteristics analysis. The Correlation Coefficient gives a summary view that tells us whether a relationship exists between clinical features and lipid species, how strong that relationship is, and whether the relationship is positive or negative. We can decide the cut-offs for the correlation coefficient and the p-value by parameter \Robject{sig\_cor\_coef} and \Robject{sig\_pvalue}. The rule of thumb in medical research recommended by Mukaka for interpreting the size of a correlation coefficient is provided below \cite{mukaka2012guide}.
...
@
\begin{figure}
  \includegraphics[width=0.8\textwidth]{images/char_Clin_Cor_table_plot.jpeg}
  \caption{\textbf{Correlation coefficient for lipid characteristics analysis}\\  Only the variables that pass the defined cut-offs for p-value and the correlation coefficient are shown on the heatmap. The rows of the heatmap are clinical features and the columns are lipid characteristics.}
  \label{fig:char_Clin_Cor_heatmap}
\end{figure}
...
@
\begin{figure}
  \includegraphics[width=0.8\textwidth]{images/char_Clin_LR_table_plot.jpeg}
  \caption{\textbf{The heatmap of linear regression for lipid characteristics analysis}\\  Only the variables that pass the user-defined cut-offs for p-value and the correlation coefficient are shown on the heatmap. The rows of the heatmap are clinical features and the columns are the lipid characteristics.}
  \label{fig:char_Clin_LR_heatmap}
\end{figure}
