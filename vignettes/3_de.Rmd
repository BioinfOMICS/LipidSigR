...
Please read `vignette("1_tool_function")` before preparing the input data.

```{r load LipidSigR, message = FALSE}
library(LipidSigR)
```

```{r load_data_profiling, include=FALSE, include=FALSE, echo=FALSE}
...
and input parameter settings. You can view the data in `subChar_se` by `LipidSigR::extract_summarized_experiment`. Please read `vignette("1_tool_function")`.
...
Please read `Dimension reduction` in `vignette("2_profiling")`
